...
main :: IO ()
main = withGCJ $ do
    [x, y, cs] <- words <$> getLine
    print $ solve (read x) (read y) cs
...
solve x y cs = uncurry min $ U.ifoldl' step (0, 0) $ U.fromList cs
    where
        step (!pc, !pj) i = \case
            'C' | i > 0 -> (min pc (pj + y), inf)
                | otherwise -> (0, inf)
            'J' | i > 0 -> (inf, min (pc + x) pj)
                | otherwise -> (inf, 0)
            _ | i > 0 -> (min pc (pj + y), min (pc + x) pj)
              | otherwise -> (0, 0)
