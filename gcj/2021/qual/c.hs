...
main = withGCJ $ do
    ...
    putStrLn . maybe "IMPOSSIBLE" (unwords.map show)
        $ solve n c
...
solve n c
    | c < n - 1 = Nothing
    | c <= n + (n - 2)
    , k <- c - (n - 2) = Just $ reverse [1..k] ++ [k+1..n]
    | otherwise = reverse . (1:) . map succ
        <$> solve (n - 1) (c - n)

calcCost :: [Int] -> Int
calcCost [_] = 0
calcCost xs = 1 + length ys + calcCost (map pred (reverse ys ++ zs))
    where
        (ys, _:zs) = span (/=1) xs

prop :: Int -> Int -> Bool
prop n c
    | not (2 <= n && n <= 100 && 1 <= c && c <= 1000) = True
    | otherwise = case solve n c of
        Just xs -> calcCost xs == c
        Nothing -> True
