...
solve (V.map (U.map digitToInt . U.unfoldrN m C.uncons) -> mat)
    = succ . V.maxIndex
    $ V.map (score prob) mat
    where
        prob :: U.Vector Double
        prob = U.map  (\freq -> fromIntegral freq / fromIntegral n)
            $ V.foldl' (U.zipWith (+)) (U.replicate n 0) mat

score :: U.Vector Double -> U.Vector Int -> Double
score qs ps = U.sum $ U.zipWith f ps qs
    where
        f 0 q = 0.0
        f 1 q = log (1/q)
