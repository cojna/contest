...
main :: IO ()
main = withGCJ $ do
    [x, y, moves] <- words <$> getLine
    case solve (read x) (read y) moves of
        Just res -> print res
...
solve 0 0 _ = undefined
solve x y moves = go 1 (Pos x y) moves
  where
    go !t (Pos x y) (d:ds)
        | reachable t pos' = Just t
        | otherwise = go (t + 1) pos' ds
      where
        !pos' = case d of
            'N' -> Pos x (y + 1)
            'E' -> Pos (x + 1) y
...
reachable :: Int -> Pos -> Bool
reachable t (Pos x y) = abs x + abs y <= t
