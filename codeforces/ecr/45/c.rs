...
const LIM: usize = 300_010;

fn solve<R: std::io::Read, W: std::io::Write>(io: &mut IO<R, W>) {
    let n: usize = io.read();
    let mut m: Vec<isize> = Vec::with_capacity(n);
    let mut g: Vec<isize> = Vec::with_capacity(n);
    let mut freq: HashMap<isize, u64> = HashMap::new();
    for _ in 0..n {
        let cs: Vec<char> = io.read();
        let mut acc = 0;
        let mut acc_min = INF as isize;
        for &c in &cs {
            if c == '(' {
                acc += 1;
            } else {
        ...
        g.push(acc);
        if acc == acc_min {
            let f = freq.entry(acc).or_insert(0);
            *f += 1;
        }
    }
    let mut res: u64 = 0;
