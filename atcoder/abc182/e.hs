...
main :: IO ()
main = do
    [h, w, n, m] <- map read.words <$> getLine
    (xs, ys) <- U.splitAt n . U.unfoldrN (n + m) (runParser $ (,) <$> int1 <*> int1) <$> C.getContents
    print $ solve h w n m xs ys

solve :: Int -> Int -> Int -> Int -> U.Vector (Int, Int) -> U.Vector (Int, Int) -> Int
solve h w n m ls bs = U.length . U.filter id
    $ U.zipWith (||)
        (solve' h w n m ls bs)
        (transpose w h $ solve' w h n m (U.map swap ls) (U.map swap bs))

solve' :: Int -> Int -> Int -> Int -> U.Vector (Int, Int) -> U.Vector (Int, Int) -> U.Vector Bool
solve' h w n m ls bs = U.create $ do
    mat <- UM.replicate (h * w) False
    checked <- UM.replicate (h * w) False
    rep h $ \x -> do
        rep w $ \y -> do
            let xy = ix x y
            flg <- UM.unsafeRead checked xy
            when (not flg && not (U.unsafeIndex block xy)) $ do
                fix (\loop !found !y' -> do
                    let !xy' = ix x y'
                    if found
                    then do
                        if y' < w && not (U.unsafeIndex block xy')
                        then do
                            UM.unsafeWrite checked xy' True
                            UM.unsafeWrite mat xy' True
                            loop True (y' + 1)
                        else return ()
                    else do
                        if y' < w && not (U.unsafeIndex block xy')
                        then do
                            UM.unsafeWrite checked xy' True
                            if U.unsafeIndex light xy'
                            then do
                                flip MS.mapM_ (stream y (y' + 1)) $ \j -> do
                                    UM.unsafeWrite mat (ix x j) True
                                loop True (y' + 1)
                            else loop False (y' + 1)
                        else return ()
                    ) False y
  ...
  where
    ...
    isBlock x y = U.unsafeIndex block (ix x y)
    {-# INLINE isBlock #-}

    !block = U.create $ do
        mat <- UM.replicate (h*w) False
        U.forM_ bs $ \(x,y)->do
            UM.unsafeWrite mat (ix x y) True
        return mat
    !light = U.create $ do
        mat <- UM.replicate (h*w) False
        U.forM_ ls $ \(x,y)->do
...
transpose :: (U.Unbox a) => Int -> Int -> U.Vector a -> U.Vector a
transpose h w m = U.create $ do
    mat <- UM.unsafeNew (h * w)
    rep w $ \i -> do
        rep h $ \j -> do
            UM.unsafeWrite mat (i * h + j)
                $ U.unsafeIndex m (j * w + i)
    return mat
