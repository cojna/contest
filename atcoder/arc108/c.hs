...
main = do
    ...
    putStr.unlines.map show.U.toList$ solve n m es

solve :: Int -> Int -> U.Vector (Int, Int, Int) -> U.Vector Int
solve n m es = runST $ do
    cs <- UM.replicate (n + 1) nothing
    fix (\dfs pv v elabel -> do
        cv <- UM.read cs v
        when (cv == nothing) $ do
            plabel <- UM.read cs pv
            if plabel /= elabel
            then do
                UM.write cs v elabel
            else if elabel /= 1
            then do
                UM.write cs v 1
            else do
                UM.write cs v 2
            U.forM_ (gr `adjW` v) $ \(nv, label) -> do
                dfs v nv label
        ) n root nothing
    U.init <$> U.freeze cs
  where
