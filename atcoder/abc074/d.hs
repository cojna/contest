...
main = do
    n <- readLn @Int
    mat <- U.unfoldrN (n*n) (runParser int) <$> C.getContents
    print . maybe (-1) id $ solve n mat
...
solve n mat
    | mat /= U.modify (warshallFloyd n) mat = Nothing
    | otherwise = runST $ do
        d <- U.thaw mat
        rep n $ \k -> do
            rep n $ \i -> do
                rep n $ \j -> when (k/=i && k/=j && i/=j) $ do
                    dij <- UM.unsafeRead d (i * n + j)
                    dik <- UM.unsafeRead d (i * n + k)
                    dkj <- UM.unsafeRead d (k * n + j)
                    when (dij >= dik + dkj) $ do
                        UM.unsafeWrite d (i * n + j) inf
        Just.(`div`2).U.sum.U.filter (/= inf) <$> U.unsafeFreeze d
