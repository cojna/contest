...
solve n (radixSort64 -> xs) = U.sum $ U.imap calc xs
  where
    calc !i x
        | r > 0 = (2 ^ (2 * l)) * 2 * (2 ^ r) * IntMod x * (IntMod r * 2 ^ (r - 1) + 2 ^ r)
        | otherwise = (2 ^ (2 * l)) * 2 * IntMod x
      where
        !l = i
        !r = n - i - 1

radixSort64 :: U.Vector Int -> U.Vector Int
radixSort64 v = F.foldl' step v [0, 16]
  where
