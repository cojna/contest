...
solve n m xs
    | Just l <- calcLCM m $ U.map (`quot` 2) xs
    , U.all (\x -> odd $ quot l (quot x 2)) xs
        = succ . upperBound 0 m $ \i ->
            l * (2 * i + 1) <= m
    | otherwise = 0

calcLCM :: Int -> U.Vector Int -> Maybe Int
calcLCM lim xs = U.foldl' step (Just 1) xs
  where
