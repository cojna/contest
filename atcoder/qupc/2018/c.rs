...
fn solve<R: std::io::BufRead, W: std::io::Write>(io: &mut IO<R, W>) {
    let (h, w, d_limit): (usize, usize, usize) = io.readln();
            ...
            if m[i][j] == '@' {
                ...
                while let Some(((x, y), d)) = q.pop_front() {
                    ...
                    for k in 0..4 {
                        ...
                        if nx < h && ny < w && m[nx][ny] == '.' || m[nx][ny] == 'G' {
                            m[nx][ny] = '#';
    ...
    let mut start = (NOTHING, NOTHING);
    let mut goal = (NOTHING, NOTHING);
    ...
    if goal == (NOTHING, NOTHING) {
        io.writeln(-1);
        return;
    }

    let mut dist: Vec<Vec<usize>> = vec![vec![NOTHING;w];h];
    ...
    while let Some((x, y)) = q.pop_front() {
        for k in 0..4 {
            let nx = x.wrapping_add(DX[k]);
            let ny = y.wrapping_add(DY[k]);
            if nx < h && ny < w && m[nx][ny] != '#' && dist[nx][ny] == NOTHING {
    ...
    if dist[goal.0][goal.1] == NOTHING {
        io.writeln(-1);
    } else {
        io.writeln(dist[goal.0][goal.1]);
    }
}
