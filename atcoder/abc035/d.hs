...
main = do
    ...
    print $ solve n m t xs es


solve :: Int -> Int -> Int -> U.Vector Int -> U.Vector (Vertex, Vertex, Int) -> Int
solve n m t xs es = U.maximum $ U.zipWith3 calc xs from0 to0
  where
    root = 0
    calc x from to
        | from < maxBound, to < maxBound = x * max 0 (t - from - to)
        | otherwise = 0
    from0 = dijkstraCSR root $ buildDirectedGraphW n es
    to0 = dijkstraCSR root $ buildDirectedGraphW n $ U.map (\(x,y,z)->(y,x,z)) es
