...
main = do
    ...
    putStrLn.concatMap show.U.toList $ solve n q qs

solve :: Int -> Int -> U.Vector (Int, Int) -> U.Vector Int
solve n q qs = U.take n
    . U.postscanl' xor 0
    . U.accumulate xor (U.replicate (n + 1) 0)
    $ U.map ((,1).fst) qs U.++ U.map ((,1).snd) qs
