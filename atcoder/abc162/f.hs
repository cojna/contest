...
solve n xs = case initS n xs of
    (acc0, xs0) -> finalS n $ U.ifoldl' nextS acc0 xs0
...
initS :: Int -> U.Vector Int -> (S, U.Vector Int)
initS n xs = (S (xs U.! 0) (xs U.! 1) 0, U.drop 2 xs)

nextS :: S -> Int -> Int -> S
nextS S{..} i x
--    | traceShow (s0, s1, s2) False = undefined
    | testBit i 0 = S s0       (s1 + x)    (max s1 s2)
    | otherwise   = S (s0 + x) (max s0 s1) (s2 + x)
