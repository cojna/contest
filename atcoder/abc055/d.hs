...
main = do
    n <- readLn @Int
    cs <- getLine
    putStrLn. maybe "-1" id $ solve n cs

solve :: Int -> String -> Maybe String
solve n oxs = L.find ((== oxs) . encode) [simulate c0 c1 oxs | c0<-"SW",c1<-"SW"]

neg :: Char -> Char
neg 'S' = 'W'
neg _ = 'S'

simulate :: Char -> Char -> String -> String
simulate c0 c1 (_:oxs) = c0:c1:go c0 c1 (init oxs)
  where
    go c 'S' ('o':cs) = c : go 'S' c cs
    go c 'S' ('x':cs) = neg c : go 'S' (neg c) cs
    go c 'W' ('o':cs) = neg c : go 'W' (neg c) cs
    go c 'W' ('x':cs) = c : go 'W' c cs
    go _ _ _ = []

encode :: String -> String
encode sws@(c0:_) = go (last sws:sws)
  where
    go (x:'S':y:cs)
        | x == y = 'o' : go ('S':y:cs)
        | otherwise = 'x' : go ('S':y:cs)
    go (x:'W':y:cs)
        | x == y = 'x' : go ('W':y:cs)
        | otherwise = 'o' : go ('W':y:cs)
    go [x,'S']
        | x == c0 = "o"
        | otherwise = "x"
    go [x, 'W']
