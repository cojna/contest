...
solve _ x 1 = x
solve n x m
    | n <= 2 * m = solveNaive n x m
    | otherwise = offsetSum + q * periodSum
        + solveNaive r start m
  where
    (!start, !offset, !offsetSum, !period, !periodSum) = solve0 x m
    (q, r) = divMod (n - offset) period


solve0 :: Int -> Int -> (Int, Int, Int, Int, Int)
solve0 x m = (x0, o, os, IS.size s0 - o, IS.foldl' (+) (-os) s0)
  where
    (!x0, !s0) = go0 IS.empty x
    (!o, !os) = go 0 0 x
    go0 !s !y
        | IS.notMember y s = go0 (IS.insert y s) (y * y `rem` m)
        | otherwise = (y, s)

    go :: Int -> Int -> Int -> (Int, Int)
    go !acc !sz !y
        | y /= x0 = go (acc + y) (sz + 1) (y * y `rem` m)
        | otherwise = (sz, acc)



solveNaive :: Int -> Int -> Int -> Int
solveNaive n x m = go 0 n x
  where
    go !acc 0 _ = acc
    go !acc !rest !y = go (acc + y) (rest - 1) (y * y `rem` m)
