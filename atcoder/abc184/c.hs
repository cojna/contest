...
main :: IO ()
main = do
    [r1,c1] <- map read.words <$> getLine
    [r2,c2] <- map read.words <$> getLine
    print $ solve (r1, c1) (r2, c2)

solve :: (Int, Int) -> (Int, Int) -> Int
solve p1@(r1, c1) p2@(r2, c2) = solve' (r1-r2, c1-c2)
...
solve' (x, y)
    ...
    | abs x == abs y = 1
    | abs x + abs y <= 3 = 1
    | mod (abs x) 2 == mod (abs y) 2 = 2
    | not$null[()|nx<-[-3..3],ny<-[-3..3],abs nx + abs ny <= 3,abs(x-nx)==abs(y-ny)] = 2
    | abs x + abs y <= 6 = 2
    | otherwise = 3
