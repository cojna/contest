...
solve a b c = calc $ U.create $ do
    dp <- UM.replicate (shiftL 1 21) 0.0
    UM.write dp (ix a b c) 1.0
    flip MS.mapM_ (stream a 100) $ \x -> do
        flip MS.mapM_ (stream b 100) $ \y -> do
            flip MS.mapM_ (stream c 100) $ \z -> do
                p <- UM.unsafeRead dp (ix x y z)
                UM.unsafeModify dp (+ (p * (fromIntegral x/fromIntegral (x+y+z)))) (ix (x + 1) y z)
                UM.unsafeModify dp (+ (p * (fromIntegral y/fromIntegral (x+y+z)))) (ix x (y + 1) z)
                UM.unsafeModify dp (+ (p * (fromIntegral z/fromIntegral (x+y+z)))) (ix x y (z + 1))
    return dp
  where
    ix x y z = unsafeShiftL x 14 .|. unsafeShiftL y 7 .|. z
    {-# INLINE ix #-}
    !s0 = a + b + c
    calc prob = sum $ do
        x <- [0..100]
        ...
        return $! fromIntegral(x + y + z - s0) * U.unsafeIndex prob (ix x y z)
