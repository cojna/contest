...
main = do
    bs <- C.getLine
    let n = C.length bs
    print $ solve n $ U.unfoldrN n (runParser $ fmap digitToInt char) bs


solve :: Int -> U.Vector Int -> Int
solve n ds = pred $ lowerBound 1 (n + 1) (not.validate)
  where
    validate k
        | 2 * k <= n = True
        | otherwise = same $ U.slice (n - k) (n - 2 * (n - k)) ds
    same v = U.null v || U.all (== U.head v) v
