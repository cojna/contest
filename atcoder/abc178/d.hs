...
solve :: Int -> IntMod
solve lim = runST $ do
    dp <- UM.replicate (ix maxLen 2048) 0
    UM.write dp (ix 0 0) 1
    cum <- UM.replicate 2049 1
    flip MS.mapM_ (stream 1 maxLen) $ \i -> do
        flip MS.mapM_ (stream 3 2048) $ \j -> do
            s <- UM.read cum (j - 3)
            UM.modify dp (+s) (ix i j)
        UM.set cum 0
        when (3 * i < 2049) $ do
            UM.write cum (3 * i) 1
        flip MS.mapM_ (stream 1 2049) $ \j -> do
            prev <- UM.read cum (j - 1)
            cur <- UM.read dp (ix i j)
            UM.write cum j $ prev + cur
    MS.foldl' (+) 0
        . MS.mapM (\i -> UM.read dp (ix i lim))
        $ stream 0 maxLen
  where
    ix :: Int -> Int -> Int
    ix i j = i * 2048 + j
