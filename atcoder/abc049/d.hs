...
main :: IO ()
main = do
    [n, k, l] <- map read.words <$> getLine
    (pqs, rss) <- U.splitAt k . U.unfoldrN (k + l) (runParser $ (,) <$> int1 <*> int1) <$> C.getContents
    putStr.unwords.map show.U.toList $ solve n pqs rss

solve :: Int -> U.Vector (Int, Int) -> U.Vector (Int, Int) -> U.Vector Int
solve n pqs rss = runST $ do
    uf0 <- newUnionFind n
    U.forM_ pqs $ \(p,q) -> do
        void $ uniteUF uf0 p q
    uf1 <- newUnionFind n
    U.forM_ rss $ \(r,s) -> do
        void $ uniteUF uf1 r s

    encoded <- UM.unsafeNew n
    rep n $ \i -> do
        encode <$> findUF uf0 i <*> findUF uf1 i
            >>= UM.unsafeWrite encoded i
    Intro.sort encoded
    U.generateM n $ \i -> do
        e <- encode <$> findUF uf0 i <*> findUF uf1 i
        l <- lowerBoundM 0 n $ \j ->
            (e<=) <$> UM.unsafeRead encoded j
        r <- lowerBoundM 0 n $ \j ->
            (e<) <$> UM.unsafeRead encoded j
        return $! r - l
