...
main = do
    ...
    mat <- replicateM n $ do
        U.toList.U.unfoldrN m (runParser int) <$> C.getLine
    print $ solve n m mat

solve :: Int -> Int -> [[Int]] -> Int
solve n m mat = go maxBound m mat
  where
    go !acc 0 _ = acc
    go !acc !sz mat = case extract mat of
        (len, x) -> go (min acc len) (sz - 1) (map (L.delete x) mat)
