...
solve n xs = U.sum $ runST $ do
    seg <- buildSegTree $ U.map (coerce @Int @(Min Int)) xs
    flip U.imapM xs $ \i x -> do
        l <- minLeftSegTree seg (i + 1) (== (Min x))
        r <- min n <$> maxRightSegTree seg i (== (Min x))
        return $! (i - l + 1) * (r - i) * x
