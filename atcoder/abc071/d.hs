...
parse (x:y:s)
    | x == y = True : parse s
    | otherwise = False : parse (y:s)
parse [_] = [False]
parse [] = []


solve :: Int -> String -> String -> IntMod
solve n (parse -> cs0) _ = case cs0 of
    (True:_) -> go 6 cs0
    (False:_) -> go 3 cs0
  where
    go !acc (True:True:cs) = go (3*acc) (True:cs)
    go !acc (True:False:cs) = go acc (False:cs)
    go !acc (False:True:cs) = go (2*acc) (True:cs)
    go !acc (False:False:cs) = go (2*acc) (False:cs)
    go acc _ = acc
