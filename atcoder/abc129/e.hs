...
solve :: C.ByteString -> IntMod
solve bs = go (2^%C.count '1' bs) (C.length bs) 0 $ C.unpack bs
  where
    go !res !len !ones ('1':cs) = go (res +% (2^%ones) *% (3 ^% (len-1))) (len-1) (ones+1) cs
    go !res !len !ones ('0':cs) = go res (len-1) ones cs
    go res _ _ [] = res
