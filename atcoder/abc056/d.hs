...
solve n k (U.modify Intro.sort . U.map (min k) -> xs) = lowerBound 0 n isNeed
  where
    isNeed :: Int -> Bool
    isNeed key = runST $ do
        dp <- UM.replicate (2 * k) False
        UM.write dp 0 True
        flip U.imapM_ xs $ \i x -> when (i /= key && x < k) $ do
            rev k $ \j -> do
                dpj <- UM.unsafeRead dp j
                UM.unsafeModify dp (|| dpj) $ j + x
        MS.or . MS.mapM (UM.unsafeRead dp) $ stream (k - xs U.! key) k
