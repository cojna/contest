...
solve n m l = solve' n m l - solve' n m (l - 1)

solve' :: Int -> Int -> Int -> IntMod
solve' n m l = runST $ do
    dp <- UM.replicate ((n + 1) * (m + 1)) 0
    UM.write dp (ix 0 0) 1
    rep1 n $ \v -> do
        rep (m + 1) $ \e -> do
            rep1 (min l v) $ \i -> do
                when (e - i >= 0) $ do
                    x <- UM.read dp (ix (v - i) (e - i))
                    let c = comb (n - (v - i) - 1) (i - 1)
                    let k | i > 2 = fact (i - 1) * inv2
                          | otherwise = IntMod (i - 1)
                    UM.unsafeModify dp (+(c*k*x)) (ix v e)
                when (e - i + 1 >= 0) $ do
                    x <- UM.read dp (ix (v - i) (e - i + 1))
                    let c = comb (n - (v - i) - 1) (i - 1)
                    let k | i > 1 = fact i * inv2
                          | otherwise = 1
                    UM.unsafeModify dp (+(c*k*x)) (ix v e)
    UM.read dp (ix n m)
  where
