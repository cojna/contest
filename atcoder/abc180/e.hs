...
main = do
    n <- readLn @Int
    ps <- U.unfoldrN n (runParser $ (,,) <$> int <*> int <*> int) <$> C.getContents
    print $ solve n ps

solve :: Int -> U.Vector (Int, Int, Int) -> Int
solve n ps = runST $ do
    dp <- UM.replicate (n * shiftL 1 n) 0x3f3f3f3f
    UM.write dp (ix (fromList [start]) start) 0
    rep (shiftL 1 n) $ \s0 -> do
        let s = coerce s0
        rep n $ \v -> do
            dpvs <- UM.unsafeRead dp (ix s v)
            rep n $ \nv -> do
                let !dnv = dpvs + dist v nv
                UM.unsafeModify dp (min dnv) (ix (insertBS nv s) nv)

    UM.read dp (ix (fromList[0..n-1]) start)
  where
    start = 0
    ix :: BitSet -> Int -> Int
    ix i j = unsafeCoerce i * n + j
    dist i j = U.unsafeIndex mat (i * n + j)
    !mat = U.create $ do
        m <- UM.replicate (n * n) 0
        rep n $ \i -> do
            rep n $ \j -> do
                UM.write m (i * n + j) $ distance (ps U.! i) (ps U.! j)
        return m



distance :: (Int, Int, Int) -> (Int, Int, Int) -> Int
distance (a, b, c) (p, q, r) = abs (p - a) + abs (q - b) + max 0 (r - c)
