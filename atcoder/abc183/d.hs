...
main = do
    ...
    stps <- U.unfoldrN n (runParser $ (,,) <$> int <*> int <*> int) <$> C.getContents
    putStrLn . bool "No""Yes" $ solve n w stps
...
solve n w stps
    = U.all (<=w)
    . U.scanl' (+) 0
    . U.accumulate (+) (U.replicate lim 0)
    $  U.map (\(s,t,p)->(s,p)) stps
        U.++ U.map (\(s,t,p)->(t,-p)) stps
