...
solve h w mat = runST $ do
    dp <- UM.replicate (h * w) 0
    dpR <- UM.replicate (h * w) 0
    dpD <- UM.replicate (h * w) 0
    dpRD <- UM.replicate (h * w) 0
    UM.write dp (ix 0 0) 1
    UM.write dpR (ix 0 0) 1
    UM.write dpD (ix 0 0) 1
    UM.write dpRD (ix 0 0) 1
    rep h $ \x -> do
        rep w $ \y -> when (mat U.! ix x y == '.') $ when (x>0||y>0) $ do
            dpu <- if x > 0 then UM.unsafeRead dpD (ix (x - 1) y) else pure 0
            dpl <- if y > 0 then UM.unsafeRead dpR (ix x (y - 1)) else pure 0
            dpul <- if x > 0 && y > 0 then UM.unsafeRead dpRD (ix (x - 1) (y - 1)) else pure 0
            let xy = ix x y
            let dpxy = dpu + dpl + dpul
            UM.unsafeWrite dp xy dpxy
            UM.unsafeWrite dpD xy (dpxy + dpu)
            UM.unsafeWrite dpR xy (dpxy + dpl)
            UM.unsafeWrite dpRD xy (dpxy + dpul)
    UM.read dp (ix (h - 1) (w - 1))
  where
    ix x y = x * w + y
    {-# INLINE ix #-}
