...
solve n xs
    = unId
    . MS.foldl' max minBound
    . MS.map (aoki n xs)
    $ stream 0 n

aoki :: Int -> U.Vector Int -> Int -> Int
aoki n xs t
    = snd
    . unId
    . MS.foldl' max ((minBound, Down maxBound), minBound)
    . MS.map (\(i, (scoreA, scoreT)) -> ((scoreA, Down i), scoreT))
    . MS.indexed
    . MS.map calc
    . MS.filter (/= t)
    $ stream 0 n
  where
    calc a = U.ifoldl' step (0, 0)
        $ slice (min a t) (max a t) xs
    step (!scoreA, !scoreB) i x
        | i .&. 1 == 1 = (scoreA + x, scoreB)
        | otherwise = (scoreA, scoreB + x)


slice :: Int -> Int -> U.Vector Int -> U.Vector Int
slice l r = U.slice l (r - l + 1)
