...
main = do
    ...
    vws <- replicateM n $ do
        [v,w] <- words <$> getLine
        return (map digitToInt v, C.pack w)
    mapM_ C.putStrLn $ solve k n vws

solve :: Int -> Int -> [([Int], C.ByteString)] -> [C.ByteString]
solve k n vws
    = maybe [] V.toList
    . (>>= construct)
    . L.find validate
  ...
  where
    validate :: [Int] -> Bool
    validate ls = maybe False (const True) $ construct ls

    construct :: [Int] -> Maybe (V.Vector C.ByteString)
    construct ls
        | any (not.fastValidate) vws = Nothing
        | otherwise  = runST $ do
            dict <- VM.replicate (k+1) C.empty
            let query :: [(Int, C.ByteString)]
                query = do
                    (digits, encoded) <- vws
                    maybe [] id $ evalStateT (decode digits) encoded
            flip fix query $ \loop -> \case
                ((d, bs):rest) -> do
                    now <- VM.read dict d
                    if bs == now
                    then loop rest
                ...
                [] -> Just <$> V.freeze (VM.tail dict)
      where
        !lens = U.fromListN (k + 1) $ 0:ls

        fastValidate (nums, encoded) = sum (map (lens U.!) nums) == C.length encoded

        decode :: [Int] -> Parser [(Int, C.ByteString)]
        decode digits = do
            flip mapM digits $ \d -> do
                (xs, ys) <- gets $ C.splitAt (lens U.! d)
                put ys
                return (d, xs)
