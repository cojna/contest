...
main = do
    ...
    putStrLn.bool"No""Yes" $ solve n m es

solve :: Int -> Int -> U.Vector (Int, Int, Int) -> Bool
solve n m es = case topSort gr of
    Nothing -> False
    Just topOrd ->  runST $ do
        pos <- UM.replicate n (-1)
        fmap U.and . U.forM topOrd $ \v -> do
            UM.modify pos (max 0) v
            vPos <- UM.read pos v
            fmap U.and . U.forM (adjW gr v) $ \(nv, d) -> do
                nvPos <- UM.read pos nv
                if nvPos < 0 || vPos + d == nvPos
                then do
                    UM.write pos nv (vPos + d)
                    return True
                else return False
  where
    !gr = buildDirectedGraphW n es
...
topSort gr = runST $ do { let { n = numVerticesCSR gr}; q <- newVecQueue n; let { inDegree = U.unsafeAccumulate (+) (U.replicate n (0 :: Int)) . U.map (flip (,) 1) $ adjacentCSR gr}; U.mapM_ (flip enqueueVQ q . fst) . U.filter ((== 0) . snd) $ U.indexed inDegree; inDeg <- U.unsafeThaw inDegree; fix $ \ loop -> do { dequeueVQ q >>= \case { Just v -> do { U.forM_ (gr `adj` v) $ \ u -> do { UM.unsafeRead inDeg u >>= \case { 1 -> enqueueVQ u q; i -> UM.unsafeWrite inDeg u (i - 1)}}; loop}; Nothing -> return ()}}; enqueueCount <- UM.unsafeRead (intVarsVQ q) _enqueueCount; if enqueueCount == n then Just <$> U.unsafeFreeze (internalVecQueue q) else return Nothing}
