...
main :: IO ()
main = do
    [n, k, c] <- map read.words <$> getLine :: IO [Int]
    cs <- U.unfoldrN n (runParser char) <$> C.getLine
    putStr.unlines.map show $ solve n k c cs

solve :: Int -> Int -> Int -> U.Vector Char -> [Int]
solve n k c cs
    | U.length cands == k
        = U.toList . U.map (succ.fst)
        . U.filter (uncurry (==))
        $ U.zip cands cands'
    | otherwise = []
  where
    cands = U.unfoldrN n step cs
    cands' = U.reverse
        . U.map (\i -> n - 1 - i)
        . U.unfoldrN n step
        $ U.reverse cs
    step s0
        | U.null s = Nothing
        | otherwise = Just (n - U.length s, U.drop (c + 1) s)
      where
        i = maybe 0 id $ U.findIndex (== 'o') s0
        s = U.drop i s0
