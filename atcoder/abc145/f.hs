...
ix :: Int -> Int -> Int -> Int
ix i j k = 301 * (301 * i + j) + k
{-# INLINE ix #-}

solve :: Int -> Int -> U.Vector Int -> Int
solve n k0 (U.cons 0 -> hs) = runST $ do
    dp <- UM.replicate (301*301*301) 0x3f3f3f3f3f3f3f3f

    UM.write dp (ix 0 0 0) 0

    rep (n + 1) $ \i -> do
        when (i > 0) $ do
            rep i $ \j -> do
                rep (k0 + 1) $ \k -> do
                    let !d = max 0 $ hs U.! i - hs U.! j
                    x <- (+d) <$> UM.read dp (ix (i-1) j k)
                    UM.modify dp (min x) (ix i i k)
                    when (k > 0) $ do
                        UM.read dp (ix (i-1) j (k-1)) >>= UM.write dp (ix i j k)
    U.minimum <$> U.unsafeFreeze (UM.slice (ix n 0 0) (301 * 301) dp)
