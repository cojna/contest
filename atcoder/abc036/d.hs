...
solve n es = runST $ do
    dpw <- UM.replicate n 0
    dpb <- UM.replicate n 0
    fix (\dfs pv v -> do
        U.mapM_ (dfs v) . U.filter (/=pv) $ gr `adj` v
        ws <- U.mapM (UM.read dpw) . U.filter (/=pv) $ gr `adj` v
        bs <- U.mapM (UM.read dpb) . U.filter (/=pv) $ gr `adj` v
        UM.write dpw v . U.product $ U.zipWith (+) ws bs
        UM.write dpb v $ U.product ws
        ) (-1) root
    (+) <$> UM.read dpw root <*> UM.read dpb root
  where
