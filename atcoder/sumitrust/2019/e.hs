...
solve n xs = answer $ U.foldl' step (1, 0, 0, 0) xs
  where
    answer (res, r, g, b)
        = res
    step (!acc, !accR, !accG, !accB) x
        | accR == x, accG == x, accB == x = (acc * 3, accR + 1, accG, accB)
        | accR == x, accG == x            = (acc * 2, accR + 1, accG, accB)
        | accR == x, accB == x            = (acc * 2, accR + 1, accG, accB)
        | accR == x                       = (acc * 1, accR + 1, accG, accB)
        | accG == x, accB == x            = (acc * 2, accR, accG + 1, accB)
        | accG == x                       = (acc * 1, accR, accG + 1, accB)
        | accB == x                       = (acc * 1, accR, accG, accB + 1)
        | otherwise                       = (0, accR, accG, accB)
