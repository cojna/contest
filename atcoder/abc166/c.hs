...
main = do
    ...
    edges <- U.unfoldrN m (runParser $ (,) <$> int1 <*> int1) <$> C.getContents
    print $ solve n m hs edges

solve :: Int -> Int -> U.Vector Int -> U.Vector (Int, Int) -> Int
solve n m hs es = U.length.U.filter p $ U.generate n id
  where
    !gr = buildUndirectedGraph n es
    p :: Vertex -> Bool
    p v = U.all (<h).U.map (hs U.!) $ gr `adj`v
      where
        !h = hs U.! v
