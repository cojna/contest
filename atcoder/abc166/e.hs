...
solve n xs = U.sum $ U.zipWith (*) freqL freqR
  where
    ls = U.filter (<n)  $ U.imap (+) xs
    rs = U.filter (>=0) $ U.imap (-) xs
    freqL = U.accumulate (+) (U.replicate (n+1) 0)
        $ U.map ((,1)) ls
    freqR = U.accumulate (+) (U.replicate (n+1) 0)
        $ U.map ((,1)) rs
    f i x
        | i - x >= 0, xs U.! (i - x) == i - x = 1
        | otherwise = 0
