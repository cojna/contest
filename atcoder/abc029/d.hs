...
solve :: Int -> Int
solve n = go 0 0 $ show n
  where
    read' cs = read ('0':cs)

    go !res !acc "0" = res + acc
    go !res !acc "1" = res + (acc + 1)
    go !res !acc ('0':cs) = go (res + acc * (read $ '1':('0' <$ cs))) (10 * acc) cs
    go !res !acc ('1':cs)
        | acc > 0 = go (res + (read' cs + 1) + acc * (read $ '1':('0' <$ cs))) (10 * acc + 1) cs
        | otherwise = go (res + (read' cs + 1)) (10 * acc + 1) cs
    go !res acc (c:cs) = go (res + (acc + 1) * (read $ '1':('0' <$ cs))) (10 * acc + digitToInt c) cs
    go res _ _ = res
