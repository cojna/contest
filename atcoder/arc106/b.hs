...
main = do
    ...
    putStrLn.bool"No""Yes" $ solve n m xs ys es

solve :: Int -> Int -> U.Vector Int -> U.Vector Int -> U.Vector (Int, Int) -> Bool
solve n m xs ys es = runST $ do
    uf <- newUnionFind n
    U.forM_ es $ \(x, y) -> do
        uniteUF_ uf x y
    freq <- UM.replicate n 0
    flip U.imapM_ xs $ \i x -> do
        findUF uf i >>= UM.modify freq (+x)
    flip U.imapM_ ys $ \i y -> do
        findUF uf i >>= UM.modify freq (subtract y)
    U.all (==0)  <$> U.freeze freq
