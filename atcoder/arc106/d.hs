...
main = do
    ...
    putStr.unlines.map show $ solve n k xs

solve :: Int -> Int -> U.Vector Int -> [IntMod]
solve n k (U.map intMod -> xs) = [calc x|x<-[1..k]]
  where
    pows :: U.Vector IntMod
    !pows = U.foldl' (\acc x -> U.zipWith (+) acc $ U.iterateN (k + 1) (* x) 1)
        (U.replicate (k + 1) 0) xs

    calc x = (sum [comb x i * pows U.! i * pows U.! (x - i)| i<-[0..x]] - (2 ^ x) * pows U.! x) / 2
