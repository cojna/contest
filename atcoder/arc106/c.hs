...
main = do
    ...
    then do
        putStr.unlines . map (unwords.map show) $[[2*i, 2*i + 1]|i<-[1..n]]
    else if n < m + 2
    then print (-1)
    else do
        putStrLn.unwords.map show $ [1, 1000000000]
        putStr.unlines . map (unwords.map show) $[[2*i, 2*i + 1]|i<-[1..m]]
        putStr.unlines . map (unwords.map show) $[[2*m + i + 1, 1000000000 - i]|i<-[1..n-m-1]]

tak, aok :: [(Int, Int)] -> Int
tak xys = fst . F.foldl' step (0, []) $ L.sortBy (comparing snd) xys
aok xys = fst . F.foldl' step (0, []) $ L.sortBy (comparing fst) xys

isValid :: (Int, Int) -> (Int, Int) -> Bool
isValid (l0, r0) (l1, r1) = not (l0 <= r1 && l1 <= r0)

step :: (Int, [(Int, Int)]) -> (Int, Int) -> (Int, [(Int, Int)])
step (!acc, used) xy
    | all (isValid xy) used = (acc + 1, xy:used)
    | otherwise = (acc, used)

normalize :: [(Int, Int)] -> [(Int, Int)]
normalize xys = [(x,y)|(x,y)<-xys,x/=y]
