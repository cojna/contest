...
main = do
    ...
    putStrLn.maybe"UNRESTORABLE" U.toList $solve s t


solve :: U.Vector Char -> U.Vector Char -> Maybe (U.Vector Char)
solve s t = case [construct i s t| i<-[0..n-1], match (U.take m $ U.drop i s) t] of
    [] -> Nothing
    cands -> Just (minimum cands)
  where
    n = U.length s
    m = U.length t

match :: U.Vector Char -> U.Vector Char -> Bool
match xs ys
    | U.length xs /= U.length ys = False
    | otherwise = U.and $ U.zipWith (\x y -> x == '?' || x == y) xs ys

construct :: Int -> U.Vector Char -> U.Vector Char -> U.Vector Char
construct i s t
    = U.map (\c -> if c == '?' then 'a' else c)
    $ U.take i s <> t <> U.drop (i + U.length t) s
