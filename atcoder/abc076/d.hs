...
main = runSolver $ do
    n <- int
    ts <- vectorN @U.Vector n int
    vs <- vectorN @U.Vector n int
    return . showLnB $ solve n ts vs

solve :: Int -> U.Vector Int -> U.Vector Int -> Double
solve n (U.map (*2) -> ds) (U.map (*2) -> vs) = runST $ do
    mfs <- UM.replicate (tMax + 1) maxBound
    UM.write mfs 0 0
    UM.write mfs tMax 0
    U.forM_ (U.zip3 ts ds vs) $ \(o, d, v) -> do
        rep1 d $ \t -> do
            UM.modify mfs (min v) (o + t)
    U.forM_ (U.zip ts $ U.cons 0 vs) $ \(o, v) -> do
        rep1 tMax $ \t -> when (o + t <= tMax) $ do
            UM.modify mfs (min (v + t)) (o + t)
    U.forM_ (U.zip ts $ vs `U.snoc` 0) $ \(o, v) -> do
        rep (tMax + 1) $ \t -> when (o - t >= 0) $ do
            UM.modify mfs (min (v + t)) (o - t)
    fs <- U.freeze mfs
    return
        . (*0.125)
