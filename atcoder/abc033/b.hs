...
main = do
    n <- readLn @Int
    sps <- V.unfoldrN n (runParser parser) <$> C.getContents
    C.putStrLn $ solve n sps

parser :: Parser (Int, C.ByteString)
parser = do
    skipSpaces
    (xs, ys) <- gets $ C.break (== ' ')
    put ys
...
cmp :: (Int, C.ByteString) -> (Int, C.ByteString) -> Ordering
cmp = comparing (Down . fst)

solve :: Int -> V.Vector (Int, C.ByteString) -> C.ByteString
solve n (V.modify (Intro.sortBy cmp) ->sps)
    | pp > other = sb
    | otherwise = "atcoder"
  where
    (pp, sb) = V.head sps
    other = V.sum . V.map fst $ V.tail sps
