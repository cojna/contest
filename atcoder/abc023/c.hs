...
main = do
    ...
    n <- readLn
    rcs <- U.unfoldrN n (runParser $ (,) <$> int1 <*> int1) <$> C.getContents
    print $ solve r c k n rcs

solve :: Int -> Int -> Int -> Int -> U.Vector (Int, Int) -> Int
solve r c k n (U.unzip -> (rs, cs)) = runST $ do
    ff <- UM.replicate 100100 0
    U.forM_ freqC $ \fj -> do
        UM.unsafeModify ff (+1) fj
    MS.foldl' (+) 0 $ flip MS.mapM (stream 0 r) $ \i -> do
        let ps = r2c V.! i
        forM_ ps $ \p -> do
            UM.unsafeModify ff (subtract 1) $ freqC U.! p
            UM.unsafeModify ff (+1) $ freqC U.! p - 1
        res <- if length ps <= k
               then UM.read ff (k - length ps)
               else return 0
        forM_ ps $ \p -> do
            UM.unsafeModify ff (+1) $ freqC U.! p
            UM.unsafeModify ff (subtract 1) $ freqC U.! p - 1
        return res
  where
    !freqC = U.accumulate (+) (U.replicate c 0)
        $ U.map (,1::Int) cs

    r2c :: V.Vector [Int]
    !r2c = V.accumulate (flip (:)) (V.replicate r [])
        . U.convert
        $ U.zip rs cs
