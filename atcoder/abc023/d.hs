...
main = do
    n <- readLn @Int
    hss <- U.unfoldrN n (runParser $ (,) <$> int <*> int) <$> C.getContents
    print $ solve n hss

solve :: Int -> U.Vector (Int, Int) -> Int
solve n hss = lowerBound lb (score hss) $ \lim ->
        U.and . U.imap (<=)
            . bucketSort (n + 1)
            $ U.map (\(h, s) -> min n $ quot (lim - h) s) hss
  where
    lb = U.maximum $ U.map fst hss
    score = U.maximum . U.imap (\i (h, s) -> h + s * i)
