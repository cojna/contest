...
main = do
    ...
    k <- maybe (-1) fst . C.readInteger <$> C.getLine
    xs <- U.unfoldrN n (runParser int1) <$> C.getLine
    print $ solve n a k xs

solve :: Int -> Int -> Integer -> U.Vector Int -> Int
solve n (pred -> a) k0 xs = 1 + L.foldl' step a [0..63]
  where
    !doublingTable = V.iterateN 64 (join U.backpermute) xs
    step pos i
        | testBit k i = doublingTable V.! i U.! pos
        | otherwise = pos

    k :: Word64
    !k = runST $ do
        dist <- UM.replicate n (-1)
        fix (\loop !i !pos -> do
            d <- UM.read dist pos
            if d < (0 :: Int)
            then UM.write dist pos i >> loop (i + 1) (xs U.! pos)
            else do
                let d' = fromIntegral d
                    i' = fromIntegral i
                if k0 <= d'
                then return $ fromIntegral k0
                else return $ fromIntegral (d' + rem (k0 - d') (i' - d'))
            ) (0 :: Int) a
