...
main :: IO ()
main = runSolver $ do
    [n, m] <- replicateM 2 int
    xs <- vectorN @U.Vector n int
    ys <- vectorN @U.Vector m int
    return . showB $ solve n m xs ys

solve :: Int -> Int -> U.Vector Int -> U.Vector Int -> Int
solve n m xs ys = runST $ do
    dp <- UM.replicate (n * m) (n + m)
    if xs U.! 0 == ys U.! 0
    then UM.write dp (ix 0 0) 0
    else UM.write dp (ix 0 0) 1
    rep n $ \i -> do
        rep m $ \j -> do
            when (i > 0) $ do
                x <- UM.read dp (ix (i - 1) j)
                UM.modify dp (min (x + 1)) (ix i j)
            when (j > 0) $ do
                y <- UM.read dp (ix i (j - 1))
                UM.modify dp (min (y + 1)) (ix i j)
            when (i > 0 && j > 0) $ do
                    z <- UM.read dp (ix (i - 1) (j - 1))
                    UM.modify dp (min (z + 1)) (ix i j)
            when (xs U.! i == ys U.! j) $ do
                if i == 0
                then UM.modify dp (min j) (ix i j)
                else if j == 0
                then UM.modify dp (min i) (ix i j)
                else do
                    z <- UM.read dp (ix (i - 1) (j - 1))
                    UM.modify dp (min z) (ix i j)
    UM.read dp (ix (n - 1) (m - 1))
    where
        ix x y = x * m + y
