...
main :: IO ()
main = runSolver $ do
    [n, m, t] <- replicateM 3 int
    xys <- vectorH @U.Vector m ((,) <$> int <*> int)
    return . yesnoB $ solve n m t xys

solve n m t xys = go n 0 $ U.toList xys
    where
        go !acc !pos ((x,y):rest)
            = (acc' > 0) && go (min n $ acc' + y - x) y rest
            where
                !acc' = acc - (x - pos)
        go acc pos [] = acc - (t - pos) > 0
