...
main = runSolver $ do
    ...
    xs <- vectorN @U.Vector m int
    return . showB $ solve n m xs

solve :: Int -> Int -> U.Vector Int -> Int
solve n m (radixSort -> xs)
    | U.null ds || k == 0= 0
    | k `seq` otherwise = U.sum $ U.map (\d -> quot (d + k - 1) k) ds
    where
        ds = U.filter (>0)
            $ U.zipWith (\x y->x-y-1) (U.snoc xs (n+1)) (U.cons 0 xs)
        k = U.minimum ds
