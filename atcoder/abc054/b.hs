...
main = do
    ...
    (xs, ys) <- U.splitAt (n * n)
        . U.unfoldrN (n * n + m * m) (runParser char)
        . C.filter (not.isSpace) <$> C.getContents
    putStrLn.bool"No""Yes" $ solve n m xs ys

solve :: Int -> Int -> U.Vector Char -> U.Vector Char -> Bool
solve n m xs ys = or $ do
    i0 <- [0..n-m]
    j0 <- [0..n-m]
    return $ and $ do
        i <- [0..m-1]
        j <- [0..m-1]
        let xij = xs U.! ((i0 + i) * n + (j0 + j))
        let yij = ys U.! (i * m + j)
        return $! xij == yij
