...
solve n m es = runST $ do
    dp <- UM.replicate (shiftL 1 n * n) 0
    UM.write dp (ix 1 0) 1
    rep (shiftL 1 n) $ \visited -> do
        rep n $ \i -> when (testBit visited i) $ do
            rep n $ \j -> when (gr U.! (ix i j) && not (testBit visited j)) $ do
                cnt <- UM.unsafeRead dp (ix visited　i)
                UM.unsafeModify dp (+cnt) (ix (setBit visited j) j)
    MS.foldl' (+) 0
        . MS.mapM (UM.read dp)
        $ stream (ix (shiftL 1 n - 1) 0) (ix (shiftL 1 n - 1) n)
  where
    ix visited pos = n * visited + pos
    !gr = U.create $ do
        mat <- UM.replicate (n * n) False
        U.forM_ es $ \(a,b) -> do
            UM.write mat (a * n + b) True
            UM.write mat (b * n + a) True
        return mat
-------------------------------------------------------------------------------
-- Utils
-------------------------------------------------------------------------------
