...
main = do
    k <- readLn @Int
    print . maybe (-1) id $ solve k

solve :: Int -> Maybe Int
solve n | even n || mod n 5 == 0 = Nothing
solve 1 = Just 1
solve 3 = Just 3
solve 7 = Just 1
solve n
    | U.head dist == inf = Nothing
    | otherwise = Just $ U.head dist + 1
  where
    !gr = buildSparseGraph n $ \builder -> do
        rep n $ \i -> do
            addDirectedEdge_ builder (i, rem (10 * i + 7) n)
    !dist = bfsCSR 7 gr
...
bfsCSR source gr@CSR{..} = U.create $ do { dist <- UM.replicate numVerticesCSR inf; que <- newVecQueue numEdgesCSR; UM.write dist source 0; enqueueVQ source que; fix $ \ loop -> do { dequeueVQ que >>= \case { Just v -> do { dv <- UM.unsafeRead dist v; U.forM_ (gr `adj` v) $ \ v' -> do { visited <- (< inf) <$> UM.unsafeRead dist v'; unless visited $ do { UM.unsafeWrite dist v' $ dv + 1; enqueueVQ v' que}}; loop}; Nothing -> return ()}}; return dist}
