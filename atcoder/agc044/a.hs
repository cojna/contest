...
solve n a b c d = memoFixIntMap f (fromIntegral n)
  where
    step memo w k x = case quotRem x k of
        (q, 0) -> (+w) <$!> memo q
        (q, r) -> min
            <$> ((+(w + d * fromIntegral r)) <$!> memo (fromIntegral q))
            <*> ((+(w + d * fromIntegral (k - r))) <$!> memo (fromIntegral q + 1))
    f _ 0 = pure 0
    f _ 1 = pure d
    f memo x = F.foldl' min (fromIntegral x * d) <$!> sequence
        [ step memo a 2 x
        , step memo b 3 x
        , step memo c 5 x
        ]
