...
solve n xs0 = L.scanl' calc n [2..n]
  where
    freq = U.filter (>0)
        . U.unsafeAccumulate (+) (U.replicate (n+1) 0)
        $ U.map (flip (,) 1) xs0
    compressed :: V.Vector (U.Vector Int)
    !compressed = group $ bucketSort n freq
    calc prev k
        | U.length freq < k = 0
        | otherwise = upperBound 1 prev $ \i ->
            V.sum (V.map (\g -> min i (U.head g) * U.length g) compressed) >= i * k
