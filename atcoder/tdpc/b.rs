...
fn main() {
    ...
    let ys: Vec<i64> = readln();
    let mut dp: Vec<Vec<i64>> = vec![vec![0;m+1]; n + 1];

    let mut sum_x: Vec<i64> = vec![0;n+1];
    let mut sum_y: Vec<i64> = vec![0;m+1];

    for i in (0..n).rev() {
        sum_x[i] = xs[i] + sum_x[i + 1];
    }

    for i in (0..m).rev() {
        sum_y[i] = ys[i] + sum_y[i + 1];
    }


    for i in (0..n).rev() {
        dp[i][m] = xs[i] + sum_x[i + 1] + sum_y[m] - dp[i + 1][m];
    }
    for j in (0..m).rev() {
        dp[n][j] = ys[j] + sum_x[n] + sum_y[j + 1] - dp[n][j + 1];
    }

    for i in (0..n).rev() {
        for j in (0..m).rev() {
            dp[i][j] = max(xs[i] + sum_x[i + 1] + sum_y[j] - dp[i + 1][j],
                           ys[j] + sum_x[i] + sum_y[j + 1] - dp[i][j + 1]);
        }
    }
