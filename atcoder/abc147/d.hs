...
solve n xs = U.sum $ U.generate 60 (solve' xs)

solve' :: U.Vector Int -> Int -> IntMod
solve' xs0 i = (2 ^ i) * U.ifoldl' step 0 xs
  where
    n = U.length xs0
    xs = U.map (\x -> unsafeShiftR x i .&. 1) xs0
    !cum = U.scanl' (+) 0 xs
    step acc j x
        | x == 1    = acc + IntMod (j - cum U.! j)
        | otherwise = acc + IntMod (cum U.! j)
