...
main = do
    ...
    zs <- U.unfoldrN n (runParser int) <$> C.getLine
    print $ solve n xs ys zs

solve :: Int -> U.Vector Int -> U.Vector Int -> U.Vector Int -> Int
solve n
    (U.modify Intro.sort -> xs)
    (U.modify Intro.sort -> ys)
    (U.modify Intro.sort -> zs)
    = U.sum $ U.map ((ys' U.!) . (n-) . calc n xs ys) xs
  where
    ys' = U.scanr' (+) 0 $ U.map (calc n ys zs) ys

calc :: Int -> U.Vector Int -> U.Vector Int -> Int -> Int
calc n xs ys x
    | U.last ys <= x = 0
    | otherwise = n - lowerBound 0 (n - 1) ((x <) . U.unsafeIndex ys)
