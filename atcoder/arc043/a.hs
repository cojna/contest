...
main = do
    ...
    putStrLn . maybe "-1" (unwords.map show.(\(x, y) -> [x, y])) $ solve n a b xs

solve :: Int -> Int -> Int -> U.Vector Int -> Maybe (Double, Double)
solve n a b xs
    | m == mm = Nothing
    | otherwise = Just (p, q)
  where
    !m = U.minimum xs
    !mm = U.maximum xs
    p = fromIntegral b / fromIntegral (mm - m)
    q = fromIntegral a - p * fromIntegral (U.sum xs) / fromIntegral n
