...
solve :: Int -> Int -> Int -> Int -> Int -> Int -> IntMod
solve r c x y d l = runST $ do
        let h = x
        let w = y
        let ix i j = i * w + j
        dp <- UM.replicate (h * w) (IntMod 0)
        rep h $ \i -> do
            rep w $ \j -> do
                when (d + l <= (i + 1) * (j + 1)) $ do
                    let !e = (i + 1) * (j + 1) - d - l
                    UM.unsafeWrite dp (ix i j) $ tri e d l
                    rep (i + 1) $ \i' -> do
                        rep (j + 1) $ \j' -> do
                            unless (i' == i && j' == j) $ do
                                when (d + l <= (i' + 1) * (j' + 1)) $ do
                                    let b = IntMod (i - i' + 1) * IntMod (j - j' + 1)
                                    x <- (*b) <$> UM.unsafeRead dp (ix i' j')
                                    UM.unsafeModify dp (subtract x) (ix i j)
        (*base) <$> UM.unsafeRead dp (ix (h - 1) (w - 1))
  where
    tri a b c = fact (a + b + c) * recipFact a * recipFact b * recipFact c
    base = IntMod (r - x + 1) * IntMod (c - y + 1)
