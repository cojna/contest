...
main :: IO ()
main = runSolver $ do
    [n, c] <- replicateM 2 int
    qs <- vectorH @U.Vector n ((,,) <$> int1 <*> int <*> int)
    return . showLnB $ solve n c qs

solve :: Int ->  Int -> U.Vector (Int, Int, Int) -> Int
solve n cc qs = (\(res,_,_)->res)
    . F.foldl' step (0, 0, 0)
    . map (\g -> (fst (head g), sum $ map snd g))
    . L.groupBy ((==)`on`fst)
    . U.toList
    $ U.modify Intro.sort $ ls U.++ rs
    where
        !ls = U.map (\(a,b,c) -> (a, c)) qs
        !rs = U.map (\(a,b,c) -> (b,-c)) qs

        step (!res, !acc, !cur) (pos, x) = (res', acc', pos)
            where
                !res' = res + min cc acc * (pos - cur)
                !acc' = acc + x
