...
main = runSolver $ do
    ...
    es <- vectorH @U.Vector m ((,) <$> int1 <*> int1)
    return . showLnB $ solve n m xs es

inf :: Int
inf = 0x3f3f3f3f3f3f3f3f

solve :: Int -> Int -> U.Vector Int -> U.Vector (Int, Int) -> Int
solve n m xs es = U.maximum $ U.zipWith (-) (V.convert memo) xs
    where
        !gr = buildDirectedGraph n es

        memo :: V.Vector Int
        memo = V.generate n $
            U.foldl' max (-inf)
                . U.map (\nv -> max (xs U.! nv) (memo V.! nv))
                . adj gr
