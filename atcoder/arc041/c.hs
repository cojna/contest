...
main = do
    ...
    xds <- U.unfoldrN n (runParser $ (,) <$> int <* char <*> char) <$> C.getContents
    print $ solve n l xds

solve :: Int -> Int -> U.Vector (Int, Char) -> Int
solve n l xds = goL 0 [] $ U.toList xds
  where
    goL !acc stack ((y, 'L'):rest) =
        case stack of
            ((x, 'L'):_) ->
                let !x' = x + 1
                in goL (acc + y - x') ((x','L'):stack) rest
            (_:_) -> goL acc ((y, 'L'):stack) rest
            [] -> goL (acc + y - 1) ((1,'L'):stack) rest
    goL !acc stack (yr:rest) = goL acc (yr:stack) rest
    goL !acc stack [] = goR acc [] stack

    goR !acc stack ((y,'R'):rest) =
        case stack of
            ((x, 'R'):_) ->
                let !x' = x - 1
                in goR (acc + x' - y) ((x','R'):stack) rest
            (_:_) -> goR acc ((y,'R'):stack) rest
            [] -> goR (acc + l - y) ((l,'R'):stack) rest
    goR !acc stack (yl:rest) = goR acc (yl:stack) rest
    goR !acc stack []
        = go acc
        . map (\g@((_,d):_) -> (d, map fst g))
        $ L.groupBy ((==) `on` snd) stack

    go !acc (('R',xs):('L',ys):rest) =
        let lenX = length xs
            lenY = length ys
            d = head ys - last xs - 1
        in go (acc + d * max lenX lenY) rest
    go !acc (('L',xs):rest) = go acc rest
    go !acc [('R',xs)] = acc
    go !acc [] = acc
