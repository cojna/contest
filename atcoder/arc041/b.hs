...
main = do
    ...
    mat <- U.unfoldrN (h*w) (runParser $ fmap digitToInt char)
        . C.filter (not.isSpace) <$> C.getContents
    let res = solve h w mat
    mapM_ putStrLn [concatMap show . U.toList $ U.slice (i * w) w res |i<-[0..h-1]]

solve :: Int -> Int -> U.Vector Int -> U.Vector Int
solve h w mat0 = U.create $ do
    mat <- U.thaw mat0
    res <- UM.replicate (h * w) 0
    rep h $ \i -> do
        rep w $ \j -> do
            UM.read mat (ix i j) >>= \case
                0 -> return ()
                d -> do
                    UM.modify mat (subtract d) (ix i j)
                    UM.modify mat (subtract d) (ix (i + 1) (j - 1))
                    UM.modify mat (subtract d) (ix (i + 1) (j + 1))
                    UM.modify mat (subtract d) (ix (i + 2) j)
                    UM.write res (ix (i + 1) j) d
    return res
  where
    ix i j = i * w + j
    {-# INLINE ix #-}
