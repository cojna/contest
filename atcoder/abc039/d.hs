...
main = do
    ...
    putStrLn . maybe "impossible" (unlines.("possible":).showMat h w) $ solve h w mat

showMat :: Int -> Int -> U.Vector Char -> [String]
showMat h w mat = [U.toList $ U.slice (i * w) w mat|i<-[0..h-1]]

solve :: Int -> Int -> U.Vector Char -> Maybe (U.Vector Char)
solve h w mat0
    | mat0 == mat0' = Just mat1
    | otherwise = Nothing
  where
    ...
    !mat1 = U.create $ do
        mat <- U.thaw mat0
        rep h $ \x -> rep w $ \y ->
            when (mat0 U.! ix x y == '.') $ do
                neighbor8 x y $ \nx ny ->
                    when (inGrid h w nx ny) $ do
                        UM.unsafeWrite mat (ix nx ny) '.'
        return mat
    !mat0' = U.create $ do
        mat <- U.thaw mat1
        rep h $ \x -> rep w $ \y ->
            neighbor8 x y $ \nx ny ->
                when (inGrid h w nx ny && U.unsafeIndex mat1 (ix nx ny) == '#') $ do
                        UM.unsafeWrite mat (ix x y) '#'
        return mat
