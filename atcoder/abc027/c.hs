...
main :: IO ()
main = readLn >>= putStrLn . bool "Aoki" "Takahashi" . solve

solve :: Int -> Bool
solve n
    | even (length bits) = go0 $ tail bits
    | otherwise = go1 $ tail bits
  where
    bits = map fromEnum . dropWhile not $ map (testBit n) [63,62..0]
go0 :: [Int] -> Bool
go0 (x:y:bs)
    | x == 1 = True
    | y == 0 = False
