...
main = do
    cs <- C.getLine
    let n = C.length cs
    print $ solve n $ U.unfoldrN n (runParser char) cs

solve :: Int -> U.Vector Char -> Int
solve n cs
    = uncurry (+)
    . bimap sum (negate.sum)
    . splitAt (div cntM 2)
    . L.sortBy (flip compare)
    $ go plus minus $ U.toList cs
  where
    !plus = U.length $ U.elemIndices '+' cs
    !minus = U.length $ U.elemIndices '-' cs
    !cntM = n - plus - minus
    go !p !m ('+':cs) = go (p - 1) m cs
    go !p !m ('-':cs) = go p (m - 1) cs
    go !p !m ('M':cs) = (p - m) : go p m cs
    go _ _ _ = []
