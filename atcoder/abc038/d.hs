...
cmp = coerce (compare @(Int, Down Int))

solve :: Int -> U.Vector (Int, Int) -> Word
solve n ps = runST $ do
    seg <- newFenwickTree @_ @(Max Word) 100100
    fmap (getMax . U.foldl' mappend mempty)
        . flip U.mapM (U.modify (Intro.sortBy cmp) ps) $ \(x, y) -> do
            res <- (+1) <$> mappendTo seg y
            mappendAt seg y res
            return $! res
