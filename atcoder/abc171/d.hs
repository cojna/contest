...
main = do
    ...
    putStr.unlines.map show.U.toList $ solve n xs qs

solve :: Int -> U.Vector Int -> U.Vector (Int, Int) -> U.Vector Int
solve n xs qs = runST $ do
    res <- UM.replicate 1 (U.sum xs)
    que <- newVecQueue (U.length qs)
    freq <- UM.replicate 100100 0
    U.forM_ xs $ \x -> do
        UM.unsafeModify freq (+1) x
    U.forM_ qs $ \(b, c) -> do
        freqB <- UM.unsafeRead freq b
        UM.unsafeWrite freq b 0
        UM.unsafeModify freq (+freqB) c
        pre <- UM.unsafeRead res 0
        let post = pre - b * freqB + c * freqB
        UM.unsafeWrite res 0 post
        enqueueVQ post que
    freezeVecQueue que
