...
main = do
    k <- readLn :: IO Int
    n <- C.length  <$> C.getLine
    print $ solve k n

solve :: Int -> Int -> IntMod
solve k n = U.sum . U.generate (k + 1) $ \i ->
    comb (n + k - i - 1) (n - 1)
        * U.unsafeIndex pow25 (k - i)
        * U.unsafeIndex pow26 i

pow25 :: U.Vector IntMod
pow25 = U.scanl' (\acc x -> acc * IntMod x) 1
    $ U.replicate FACT_CACHE_SIZE 25

pow26 :: U.Vector IntMod
pow26 = U.scanl' (\acc x -> acc * IntMod x) 1
    $ U.replicate FACT_CACHE_SIZE 26
