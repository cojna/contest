...
cost (S _ cntL cntH acc) = min cntL cntH + acc

solve :: Int -> Int -> U.Vector Int -> Int
solve n t xs = cost $ U.foldl' step (S maxBound 0 0 0) xs
  where
    !d = U.maximum
        $ U.zipWith (-)
            (U.scanr1' max xs)
    ...
    step s@(S low cntL cntH acc) x
        | x < low = (S x 1 0 (min cntL cntH + acc))
        | x == low = (S low (cntL + 1) cntH acc)
        | x == low + d = (S low cntL (cntH + 1) acc)
        | otherwise = s
