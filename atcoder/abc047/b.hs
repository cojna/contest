...
main = do
    ...
    xys <- U.unfoldrN n (runParser $ (,,) <$> int <*> int <*> int)<$> C.getContents
    print $ solve w h n xys
...
solve w h n xys
    ...
    | otherwise = 0
  where
    minX = U.foldl' max 0 . U.map px $ U.filter ((==1).dir) xys
    maxX = U.foldl' min w . U.map px $ U.filter ((==2).dir) xys
    minY = U.foldl' max 0 . U.map py $ U.filter ((==3).dir) xys
    maxY = U.foldl' min h . U.map py $ U.filter ((==4).dir) xys
px (x, _, _) = x
py (_, y, _) = y
dir (_, _, a) = a
