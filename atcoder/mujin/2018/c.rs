...
fn solve<R: std::io::BufRead, W: std::io::Write>(io: &mut IO<R, W>) {
    let (n, m): (usize, usize) = io.readln();
    let g: Vec<Vec<char>> = io.readlns(n);
    let mut up: Vec<Vec<usize>> = vec![vec![0;m+1];n+1];
    let mut down: Vec<Vec<usize>> = vec![vec![0;m+1];n+1];
    let mut left: Vec<Vec<usize>> = vec![vec![0;m+1];n+1];
    let mut right: Vec<Vec<usize>> = vec![vec![0;m+1];n+1];
    ...
    for i in 0..n {
        for j in 0..m {
            ...
            res += (left[i][j] - 1) * (down[i][j] - 1);
            res += (right[i][j] - 1) * (up[i][j] - 1);
            res += (up[i][j] - 1) * (left[i][j] - 1);
            res += (down[i][j] - 1) * (right[i][j] - 1);
        }
    }
    io.writeln(res);
}
