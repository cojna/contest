...
fn solve<R: std::io::BufRead, W: std::io::Write>(io: &mut IO<R, W>) {
    let (n, k): (usize, usize) = io.readln();
    let xs: Vec<i64> = io.readlns(n);
    let mut dp: Vec<[i64; 2]> = vec![[0,-INF];n+1];
    for i in 0..n {
        dp[i + 1][0] = max(dp[i][0], dp[i][1]) + xs[i];
        if i + 1 >= k {
            dp[i + 1][1] = max(dp[i + 1 - k][0], dp[i + 1 - k][1]);
            dp[i + 1][1] = max(dp[i + 1][1], dp[i][1]);
        }
    }
    io.writeln(max(dp[n][0], dp[n][1]));
}
