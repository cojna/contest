...
fn solve<R: std::io::BufRead, W: std::io::Write>(io: &mut IO<R, W>) {
    ...
    let mut connected = UnionFind::new(n);
    let mut clique = UnionFind::new(n);
    ...
    for _ in 0..q {
        let (typ, u, v): (usize, usize, usize) = io.readln();
        match typ {
            1 => {
                let u = u - 1;
                let v = v - 1;
                edges.insert((u, v));
                edges.insert((v, u));
                let gu = connected.find(u);
                let gv = connected.find(v);
                if gu == gv {
                    continue;
                }
                if buffer[gu].len() < buffer[gv].len() {
                    buffer.swap(gu, gv);
                }

                let mut smaller = buffer[gv].clone();
                buffer[gu].append(&mut smaller);

                connected.unite(u, v);
                if connected.find(u) != gu {
                    buffer.swap(gu, gv);
                }
            }
            2 => {
                let g = connected.find(u - 1);
                for &x in &buffer[g] {
                    clique.unite(g, x);
                }
                buffer[g].clear();
            ...
            _ => {
                if edges.contains(&(u - 1, v - 1)) || clique.equiv(u - 1, v - 1) {
                    io.writeln("Yes");
                } else {
                    io.writeln("No")
                }
