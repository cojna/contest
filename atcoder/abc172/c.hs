...
main = do
    ...
    ys <- U.unfoldrN m (runParser int) <$> C.getLine
    print $ solve n m k xs ys

solve :: Int -> Int -> Int -> U.Vector Int -> U.Vector Int -> Int
solve n m k xs ys = U.maximum $ U.imap calc cumX
  where
    !cumX = U.scanl' (+) 0 xs
    !cumY = U.scanl' (+) 0 ys
    calc :: Int -> Int -> Int
    calc i cx
     | cx <= k = (+i) . upperBound 0 m $ \j ->
        U.unsafeIndex cumY j + cx <= k
     | otherwise = 0
