...
main = do
    ...
    wps <- U.unfoldrN n (runParser $ (,) <$> int <*> int) <$> C.getContents
    print $ solve n k wps

solve :: Int -> Int -> U.Vector (Int, Int) -> Double
solve n k (U.map (\(w,p)->(fromIntegral w,fromIntegral p)) -> wps)
    = upperBoundDouble 0.0 100.0 $ \x ->
        U.sum (U.take k . U.modify (Intro.sortBy (flip compare)) $ U.map (calc x) wps) >= 0.0
  where
    calc x (w, p) = w * (p - x)
