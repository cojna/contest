...
main = do
    ...
    putStr . U.foldr (\x cs-> shows x $ showChar '\n' cs) "" $ solve n k

#define MOD 1000000007
#define FACT_CACHE_SIZE 2048

solve :: Int -> Int -> U.Vector IntMod
solve n k = runST $ do
    red <- UM.replicate (2048 * 2048) (0 :: IntMod)
    blue <- UM.replicate (2048 * 2048) (0 :: IntMod)
    let ix i j = unsafeShiftL i 11 .|. j

    UM.write red (ix 0 0) 1

    rep n $ \i -> do
        rep (k + 1) $ \j -> do
            ri <- UM.unsafeRead red (ix i j)
            UM.unsafeModify red (+ ri) (ix (i + 1) j)
            UM.unsafeModify blue (+ ri) (ix (i + 1) (j + 1))
            bi <- UM.unsafeRead blue (ix i j)
            UM.unsafeModify red (+ bi) (ix (i + 1) j)
    dp <- U.zipWith (+)
            <$> U.unsafeFreeze red
            <*> U.unsafeFreeze blue

    return . U.tail $ U.generate (k + 1) $ \j ->
        if j == 0 then 0
        else comb (k - 1) (j - 1) * U.unsafeIndex dp (ix (n - k + j) j)
