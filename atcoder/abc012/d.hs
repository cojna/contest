...
solve n m es = V.minimum $ V.map U.maximum $ chunks n dist
  where
    dist = U.create $ do
        gr <- UM.replicate (n * n) 0x3f3f3f3f
        let ix i j = i * n + j
        rep n $ \i -> do
            UM.unsafeWrite gr (ix i i) 0
        U.forM_ es $ \(x, y, w) -> do
            UM.unsafeWrite gr (ix x y) w
            UM.unsafeWrite gr (ix y x) w
        warshallFloyd n gr
        return gr
...
-------------------------------------------------------------------------------
warshallFloyd :: (PrimMonad m, U.Unbox a, Num a, Ord a) => Int -> UM.MVector (PrimState m) a -> m ()
warshallFloyd n d = do { rep n $ \ i -> do { UM.unsafeWrite d (i * n + i) 0}; rep n $ \ k -> do { rep n $ \ i -> do { rep n $ \ j -> do { dij <- UM.unsafeRead d (i * n + j); dik <- UM.unsafeRead d (i * n + k); dkj <- UM.unsafeRead d (k * n + j); UM.unsafeWrite d (i * n + j) $ min dij (dik + dkj)}}}}
