...
main = do
    ...
    xs <- U.unfoldrN na (runParser int) <$> C.getLine
    ys <- U.unfoldrN nb (runParser int) <$> C.getLine
    print $ solve na nb xs ys

solve :: Int -> Int -> U.Vector Int -> U.Vector Int -> Double
solve na nb xs ys = fromIntegral u / fromIntegral v
  where
    toSet = U.foldl' (flip IS.insert) IS.empty
    u = IS.size $ toSet xs `IS.intersection` toSet ys
    v = IS.size $ toSet xs `IS.union` toSet ys
