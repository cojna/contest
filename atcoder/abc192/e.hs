...
main :: IO ()
main = runSolver $ do
    [n, m, x, y] <- replicateM 4 int
    skipSpaces
    es <- vectorH @U.Vector m ((,,,) <$> int1 <*> int1 <*> int <*> int)
    return . maybe "-1" B.intDec
        $ solve n m (x - 1) (y - 1) es

solve :: Int -> Int -> Int -> Int -> U.Vector (Int, Int, Int, Int) -> Maybe Int
solve n m start goal edges = answer $ runST $ do
    ...
    fix $ \loop -> do
        deleteFindTopBH heap >>= \case
            Just (d, v) -> do
                dv <- UM.read dist v
                when (dv == d) $ do
                    U.forM_ (gr `adjW` v) $ \(nv, (t,k)) -> do
                        dnv <- UM.read dist nv
                        let !dnv' = quot (dv + k - 1) k * k + t
                        when (dnv' < dnv) $ do
                            UM.write dist nv dnv'
                            insertBH (dnv', nv) heap
                loop
            Nothing -> return ()
    UM.read dist goal
    where
