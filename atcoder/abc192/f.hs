...
main :: IO ()
main = runSolver $ do
    [n, x] <- replicateM 2 int
    skipSpaces
    xs <- vectorN @U.Vector n int
    return . B.intDec $ solve n x xs

solve :: Int -> Int -> U.Vector Int -> Int
solve n x xs = minimum [solve' n x xs k|k<-[1..n]]

solve' :: Int -> Int -> U.Vector Int -> Int -> Int
solve' n t xs k = runST $ do
    dp <- UM.replicate ((k + 1) * k) nothing
    UM.write dp (ix 0 0) 0
    U.forM_ xs $ \x -> do
        rev k $ \cnt -> do
            rep k $ \r -> do
                dpv <- UM.unsafeRead dp (ix cnt r)
                when (dpv /= nothing) $ do
                    UM.unsafeModify dp (max (dpv + x)) (ix (cnt + 1) (rem (x + r) k))
    res <- UM.read dp (ix k (mod t k))
    if res /= nothing
    then return $! quot (t - res) k
