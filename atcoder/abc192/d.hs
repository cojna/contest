...
main = runSolver $ do
    ...
    m <- int
    return . B.intDec $ solve cs m

solve :: U.Vector Char -> Int -> Int
solve (U.map digitToInt -> ds) (toInteger -> m)
    | U.length ds == 1 = if U.head ds <= fromIntegral m then 1 else 0
    | calc ds (U.maximum ds + 1) > m = 0
    | otherwise = binarySearch (U.maximum ds + 1) maxBound
        (\i -> calc ds i > m) - (U.maximum ds + 1)

-- >>> calc (U.fromList [1..9]) 10
-- 123456789
calc :: U.Vector Int -> Int -> Integer
calc ds (toInteger -> x)
    = U.foldl' (\acc d -> acc * x + toInteger d) 0 ds
