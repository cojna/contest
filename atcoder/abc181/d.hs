...
main = do
    cs <- C.getLine
    let n = C.length cs
    putStrLn.bool"No""Yes" $ solve n $ U.unfoldrN n (runParser char) cs

solve :: Int -> U.Vector Char -> Bool
solve 1 (U.toList -> cs) = cs == "8"
solve 2 (U.toList -> cs) = any ((==0).(`rem`8).read)[cs, reverse cs]
solve n (U.modify Intro.sort -> cs) = or
   [read[ds U.! i, ds U.! j, ds U.! k]`rem`8==0|i<-[0..m-1],j<-[0..m-1],k<-[0..m-1],i/=j,j/=k,k/=i]
  where
    !m = U.length ds
    !ds = U.fromList $ concatMap (take 3) . L.group $ U.toList cs
