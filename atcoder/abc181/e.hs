...
main = do
    ...
    ws <- U.unfoldrN m (runParser int) <$> C.getLine
    print $ solve n m hs ws

solve :: Int -> Int -> U.Vector Int -> U.Vector Int -> Int
solve n m (radixSort -> hs) (radixSort -> ws)
    = U.minimum
    $ U.generate (div (n + 1) 2) calc
  where
    !pre = U.scanl' (+) 0
        . U.map (\(x,y)->y-x)
        . tuples2N (div n 2)
    ...
    !post = U.scanr' (+) 0
        . U.map (\(x,y)->y-x)
        $ tuples2N (div n 2) $ U.tail hs
    calc i
        = pre U.! i
        + post U.! i
        + min wl wr
      where
        !hi = U.unsafeIndex hs $ 2 * i
        !key = binarySearch 0 m $ (hi<=) . U.unsafeIndex ws
        !wl | key == 0 = maxBound
            | otherwise = hi - ws U.! (key - 1)
        !wr | key == m = maxBound
            | otherwise = ws U.! key - hi
