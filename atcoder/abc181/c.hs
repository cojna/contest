...
main = do
    ...
    putStrLn.bool"No""Yes"$ solve n ps

solve :: Int -> U.Vector (Int, Int) -> Bool
solve n ps = or[p (ps U.! i) (ps U.! j) (ps U.! k)|i<-[0..n-1],j<-[i+1..n-1],k<-[j+1..n-1]]
  where
    p p0 p1 p2 = area p0 p1 p2 == 0

cross (x0, y0) (x1, y1) = x0 * y1 - y0 * x1
{-# INLINE cross #-}

area (x0, y0) (x1, y1) (x2, y2) = cross (x1 - x0, y1 - y0) (x2 - x0, y2 - y0)
{-# INLINE area #-}
