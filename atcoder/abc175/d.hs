...
main = do
    ...
    ps <- U.unfoldrN n (runParser int1) <$> C.getLine
    cs <- U.unfoldrN n (runParser $ fmap fromIntegral int) <$> C.getLine
...
solve n k ps cs
    | k <= 2 * n = calc k st0
    | otherwise = calc n st0 `max` calc n (doublingStep (k - n) st0)
  where
    st0 = U.generate n (,0)

    calc :: Int -> U.Vector (Int, Int64) -> Int64
    calc x st = unId . MS.foldl1' max
        . MS.map (U.foldl1' max . U.map snd)
        $ stepN x st

    jump :: U.Vector (Int, Int64) -> (Int, Int64) -> (Int, Int64)
    jump next (pos, s) =
        let (pos', v) = next U.! pos
            !s' = s + v
        in (pos', s')

    !table = V.iterateN 32 (\next -> U.map (jump next) next) $ U.zip ps cs

    step1 :: U.Vector (Int, Int64) ->  U.Vector (Int, Int64)
    step1 = U.map (jump (table V.! 0))

    stepN :: (Monad m)
        => Int
        -> U.Vector (Int, Int64)
        -> MS.Stream m (U.Vector (Int, Int64))
    stepN x = MS.tail . MS.iterateN (x + 1) step1

    doublingStep :: Int -> U.Vector (Int, Int64) -> U.Vector (Int, Int64)
    doublingStep x st0 =
        F.foldl' (\st next -> U.map (jump next) st)
            st0
            [table V.! i|i<-[0..31],testBit x i]
