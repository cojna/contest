...
main = do
    ...
    es <- U.unfoldrN k (runParser $ (,,) <$> int1 <*> int1 <*> int) <$> C.getContents
    print $ solve r c k es

data S = S !Int !Int !Int

score (S acc1 acc2 acc3) = acc3

solve :: Int -> Int -> Int -> U.Vector (Int, Int, Int) -> Int
solve r c k (U.modify Intro.sort -> es) = runST $ do
    dp <- UM.replicate c 0
    V.forM_ (V.generate r viewRow) $ \row -> do
        void $ U.foldM'
            (\(S acc1 acc2 acc3) (_, i, v) -> do
                dpi <- UM.unsafeRead dp i
                let s' = S
                     (max acc1 $ dpi + v)
                     (max acc2 $ max dpi acc1 + v)
                     (max acc3 $ max dpi acc2 + v)
                UM.unsafeWrite dp i $ score s'
                return s'
            ) (S 0 0 0) row
        flip MS.mapM_ (stream 1 c) $ \i -> do
            x <- UM.unsafeRead dp (i - 1)
            UM.unsafeModify dp (max x) i
    UM.read dp (c - 1)
  where
    viewRow i = U.slice (offset U.! i) (offset U.! (i + 1) - offset U.! i) es
    !offset = U.scanl' (+) 0 freq
    !freq = U.accumulate (+) (U.replicate r 0)
        $ U.map (\(r,_,_)->(r, 1)) es
