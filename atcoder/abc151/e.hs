...
conv :: Int -> Word64
conv x = unsafeCoerce $ x + MOD

solve :: Int -> Int -> U.Vector Int -> IntMod
solve n k (radixSort64.U.map conv -> xs)
    = U.ifoldl' step 0 xs
  where
    step :: IntMod -> Int -> Word64 -> IntMod
    step acc i w
        = acc + resMax - resMin
      where
        !resMax
            | k - 1 <= i = unsafeCoerce w * comb i (k - 1)
            | otherwise = 0
        !resMin
            | k - 1 < n - i = unsafeCoerce w * comb (n - i - 1) (k - 1)
            | otherwise = 0
...
newtype IntMod = IntMod{getIntMod :: Int} deriving (Eq, Ord, Read)
instance Show IntMod where {show = show.getIntMod}
...
instance Num IntMod where { (+) = coerce (+%); (-) = coerce (-%); (*) = coerce (*%); abs = id; signum = const (IntMod 1); fromInteger x = IntMod . fromInteger $ mod x modulus}
