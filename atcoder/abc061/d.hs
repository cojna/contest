...
main = do
    ...
    putStrLn.maybe "inf" show $ solve n m es

solve :: Int -> Int -> U.Vector (Int, Int, Int) -> Maybe Int
solve n m es
    | dist == minBound = Nothing
    | otherwise = Just $ -dist
  where
    !dist = bellmanFord n 0 (U.map (\(x,y,z) -> (x,y,-z)) es) U.! (n - 1)
...
bellmanFord n root edges = U.create $ do { dist <- UM.replicate n maxBound; UM.write dist root 0; U.replicateM (n - 1) $ do { U.forM_ edges $ \ (src, dst, cost) -> do { dv <- UM.unsafeRead dist src; du <- UM.unsafeRead dist dst; when (dv + cost < du && dv /= maxBound) $ do { UM.unsafeWrite dist dst $ dv + cost}}}; U.forM_ edges $ \ (src, dst, cost) -> do { dv <- UM.unsafeRead dist src; du <- UM.unsafeRead dist dst; when (dv + cost < du && dv /= maxBound) $ do { UM.unsafeWrite dist dst minBound}}; U.replicateM (n - 1) $ do { U.forM_ edges $ \ (src, dst, _) -> do { dv <- UM.unsafeRead dist src; when (dv == minBound) $ do { UM.unsafeWrite dist dst minBound}}}; return dist}
