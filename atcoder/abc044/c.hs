...
solve n a xs = runST $ do
    dp <- UM.replicate (64 * 64 * 64) 0
    UM.write dp 0 1
    flip U.imapM_ xs $ \i x -> do
        rev (i + 1) $ \j -> do
            rev (64 * 64 - x) $ \k -> do
                y <- UM.read dp (encode j k)
                UM.modify dp (+y) $ encode (j + 1) (k + x)

    MS.foldl' (+) 0
        . MS.mapM (UM.read dp)
        . MS.tail
        $ MS.generate (n + 1) $ \i -> encode i (a * i)

encode :: Int -> Int -> Int
encode cnt x = unsafeShiftL cnt 12 .|. x
