...
main :: IO ()
main = do
    [n, s] <- map read.words <$> getContents
...
solve n s = resSmall <|> resLarge
  where
    resSmall = unId . MS.find ((== s) . digitSumBase n) $ stream 2 lim
    resLarge
      | n == s = Just $! n + 1
      | otherwise
          = L.find ((== s) . digitSumBase n)
          . dropWhile (< lim)
          . map (+1)
          . divisors
          $ abs (n - s)

digitSumBase :: Int -> Int -> Int
digitSumBase n base = go 0 n
  where
    go !acc !x
      | x > 0 = case quotRem x base of (q, r) -> go (acc + r) q
      | otherwise = acc
