...
impl Memo {
    pub fn new(n: usize) -> Memo {
        let mut buf = vec![NOTHING;n + 1];
        buf[0] = 0;
        Memo { memo: buf }
    }

    pub fn get(&mut self, k: usize) -> usize {
        if self.memo[k] != NOTHING {
            self.memo[k]
        } else {
            let mut acc = std::usize::MAX;
            let mut i = 1;
            while i <= k {
                acc = min(acc, self.get(k - i) + 1);
                i *= 6;
            }
            let mut i = 1;
            while i <= k {
