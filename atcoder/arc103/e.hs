...
main :: IO ()
main = runSolver $ do
    bs <- vector @U.Vector ((=='1') <$> char)
    return . maybe "-1" (unlinesB $ pairB B.intDec B.intDec )
        $ solve bs

solve :: U.Vector Bool -> Maybe (U.Vector (Int, Int))
solve bs
    | not (require bs) = Nothing
    | otherwise = pure . U.fromListN (n - 1) . go 1 [] . tail . zip [1..] $ U.toList bs
    where
        !n = U.length bs
        go !prev stack ((i,flg):is)
            | flg = (prev, i):map ((,) i) stack ++ go i [] is
            | otherwise = go prev (i:stack) is
        go prev (n' : stack) [] = (prev, n') : map ((,) n') stack
        go _ [] [] = error "unreachable"



require :: U.Vector Bool -> Bool
require bs = U.head bs && not (U.last bs)
    && bs' == U.reverse bs'
    where
        n = U.length bs
        bs' = U.init bs
