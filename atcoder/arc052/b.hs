...
main = do
    ...
    xrhs <- replicateM n $ do
        [x,r,h] <- map read.words <$> getLine
        return (x,r,h)
    qs <- U.unfoldrN q (runParser $ (,) <$> double <*> double) <$> C.getContents
    putStr.unlines.map show.U.toList $ solve n q xrhs qs

double :: Parser Double
double = fmap fromIntegral int

solve :: Int -> Int -> [(Double, Double, Double)] -> U.Vector (Double, Double) -> U.Vector Double
solve n q xrhs qs = U.map calc qs
  where
    calc ab = sum $ map (volume ab) xrhs
    volume (a, b) (x, r, h)
        | b <= x || x + h <= a = 0.0
        | a <= x, b <= x + h = (1 - ((x + h - b)/h) ^ 3) * vol0
        | b <= x + h = (((x + h - a)/h) ^ 3 - ((x + h - b)/h) ^ 3) * vol0
        | a <= x = vol0
        | otherwise = ((x + h - a)/h) ^ 3 * vol0
      where
        !vol0 = pi * r * r * h / 3
