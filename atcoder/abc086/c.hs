...
main = do
    ...
    putStrLn.bool"No""Yes" $ solve n qs

solve :: Int -> U.Vector (Int, Int, Int) -> Bool
solve n (U.cons (0, 0, 0) -> qs) = U.and $ U.zipWith validate qs (U.tail qs)
  where
    validate (t0, x0, y0) (t1, x1, y1)
        = dx + dy <= dt && even (dt - dx - dy)
      where
        !dt = t1 - t0
        !dx = abs $ x1 - x0
        !dy = abs $ y1 - y0
