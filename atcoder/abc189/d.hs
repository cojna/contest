...
main = runSolver $ do
    n <- int
    xs <- vectorH @U.Vector n ((=="AND") <$> bytestring)
    return . showLnB $ solve n xs

solve :: Int -> U.Vector Bool -> Int
solve n bs0 = go . reverse $ U.toList bs0
    where
        go (True:bs) = go bs
        go (False:bs) = go bs + 2^(length bs + 1)
        go [] = 1
