...
main = do
    ...
    xs <- U.unfoldr (runParser int) <$> C.getContents
    let (uvs,skts) = U.splitAt (2*m) xs
    let s = skts U.! 0
        k = skts U.! 1
        ts = U.drop 2 skts
    print $ solve n m xs (tuples2N m $ U.map (subtract 1)uvs) (s-1) k (U.map (subtract 1) ts)

solve n m xs uvs s0 k0 ts0 = runST $ do
    dist <- UM.replicate (numV * k) (INF :: Int)
    let ix i j = i * k + j
    UM.write dist (ix 1 0) 0
    rep numV $ \visited -> do
        rep k $ \i -> do
            when (testBit visited i) $ do
                d <- UM.read dist (ix visited i)
                rep k $ \j -> do
                    let !visited' = setBit visited j
                    d' <- UM.read dist (ix visited' j)
                    when (d + cost U.! (i * k + j) < d') $ do
                        UM.write dist (ix visited' j) (d + cost U.! (i * k + j))
    minimum <$> mapM (UM.read dist . ix (numV - 1)) [0..k-1]
  where
    gr = buildUndirectedGraph n uvs
    !ts = U.cons s0 ts0
    !tsL = U.toList ts
    dists :: V.Vector (U.Vector Int)
    dists = V.map (flip bfsCSR gr) $ U.convert ts
    !k = k0 + 1
    cost = U.fromList [dists V.! i U.! j|i<-[0..k0], j<-tsL]
    !numV = 2 ^ k
