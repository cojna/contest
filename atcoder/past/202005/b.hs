...
main = do
    ...
    qs <- U.unfoldr (runParser int) <$> C.getContents
    putStr.unlines.map show.U.toList $ solve n m qs

solve :: Int -> Int -> U.Vector Int -> U.Vector Int
solve n m qs0 = runST $ do
    que <- newVecQueue defaultVecQueueSize
    solved <- UM.replicate (n + 1) (0 :: Word)
    score <- UM.replicate (m + 1) n
    flip fix (U.toList qs0) $ \loop -> \case
        (1:x:qs) -> do
            s <- UM.read solved x
            res <- fmap sum . forM (filter (testBit s) [0..m]) $ \i -> do
                UM.read score i
            enqueueVQ res que
            loop qs
        (2:x:y:qs) -> do
            UM.modify solved (flip setBit y) x
            UM.modify score (subtract 1) y
            loop qs
        [] -> freezeVecQueue que
