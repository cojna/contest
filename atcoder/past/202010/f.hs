...
main = do
    ...
    css <- C.words <$> C.getContents
    putStrLn . maybe "AMBIGUOUS" C.unpack $ solve n k css

solve :: Int -> Int -> [C.ByteString] -> Maybe C.ByteString
solve n k css = case filter ((==cnt) . fst) dict of
    [(_, res)] -> Just res
    _ -> Nothing
  where
    (cnt, cs) = dict !! (k - 1)
    dict = L.sortOn (negate.fst)
