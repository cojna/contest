...
solve n xs = binarySearch 0 maxBound (\k -> U.or $ U.generate n (calc k))
  where
    !cum = U.scanl' (+) 0 $ xs U.++ xs
    !s = U.sum xs
    calc k i = lb < ub
      where
        !lb = binarySearch (i + 1) (i + n) $ \j ->
                s <= 2 * (cum U.! j - cum U.! i)
        !ub = binarySearch (i + 1) (i + n) $ \j ->
                s + k < 2 * (cum U.! j - cum U.! i)
