...
main = do
    k <- readLn @Int
    rows <- V.replicateM k $ do
        n <- readLn
        U.unfoldrN n (runParser int1) <$> C.getLine
    q <- readLn
    bs <- U.unfoldrN q (runParser int1) <$> C.getLine
    print $ solve k rows q bs

solve :: Int -> V.Vector (U.Vector Int) -> Int -> U.Vector Int -> IntMod
solve k rows q bs = runST $ do
    freq <- UM.replicate 20 0
    U.foldM' (\acc b -> do
        fs <- U.tail . U.scanr' (+) 0 <$> U.freeze freq
        fmap (U.foldl' (+) (acc + invs V.! b))
            . flip U.imapM (U.zip fs $ freqs V.! b)
            $ \i (fl, fr) -> do
                UM.modify freq (+fr) i
                return $! fl * fr
        ) 0 bs
  where
    !invs = V.map calc rows
    !freqs = V.map frequency rows

frequency :: U.Vector Int -> U.Vector IntMod
frequency xs = U.map IntMod
    . U.accumulate (+) (U.replicate 20 0)
    $ U.map (,1) xs


calc :: U.Vector Int -> IntMod
calc xs = runST $ do
    freq <- UM.replicate 20 (0 :: Int)
    res <- UM.replicate 1 0
    U.forM_ xs $ \x -> do
        rep 20 $ \i -> do
            when (i > x) $ do
                cnt <- UM.read freq i
                UM.modify res (+ IntMod cnt) 0
        UM.modify freq (+1) x
    UM.read res 0
