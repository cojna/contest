...
main = do
    ...
    mat <- U.unfoldrN (h*w) (runParser char) . C.filter (not.isSpace)
        <$> C.getContents
    let res = solve h w mat
    rep h $ \i -> do
        putStrLn . U.toList $ U.slice (i * w) w res

solve :: Int -> Int -> U.Vector Char -> U.Vector Char
solve h w mat = U.generate (h * w) $ \xy ->
    ...
    in head . show . sum $ do
        ...
        let !ny = y + dy
        guard $ 0 <= ny && ny < w
        return $! if mat U.! (nx * w + ny) == '#' then 1 else 0
