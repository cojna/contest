...
main = do
    ...
    [xab, xac, xbc] <- map read.words <$> getLine
    s <- C.getLine
    es <- U.unfoldrN m (runParser $ (,,) <$> int1 <*> int1 <*> int) <$> C.getContents
    print $ solve n m (xab, xac, xbc) (U.unfoldrN n (runParser char) s) es

solve :: Int -> Int -> (Int, Int, Int) -> U.Vector Char -> U.Vector (Int, Int, Int) -> Int
solve n m (xab, xac, xbc) cs es = dist U.! goal
  where
    ...
    oparse 'B' = ob
    oparse 'C' = oc
    !dist = dijkstraCSR start
        . buildDirectedGraphW (n + 6)
        $ es U.++ U.map (\(x,y,z)->(y,x,z)) es
            U.++ U.imap (\i c -> (i, iparse c, 0)) cs
            U.++ U.imap (\i c -> (oparse c, i, 0)) cs
            U.++ U.fromListN 6
                [ (ia, ob, xab), (ib, oa, xab)
                , (ib, oc, xbc), (ic, ob, xbc)
                , (ic, oa, xac), (ia, oc, xac)
