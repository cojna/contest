...
main = do
    ...
    es <- U.unfoldrN m (runParser edgeP) <$> C.getContents
    print . maybe (-1) id $ solve n m es

edgeP :: Parser (Int, Int)
edgeP = do
    modify' $ C.dropWhile isSpace
    bs <- bytestring
    c <- int
    return (C.foldl' (\acc c -> 2 * acc + if c == 'Y' then 1 else 0) 0 bs, c)

bytestring :: Parser C.ByteString
bytestring = StateT $ \bs -> do
    pos <- C.findIndex isSpace bs
    return $ C.splitAt pos bs
...
solve n m es
    ...
    | otherwise = Nothing
  where
    !d = dist U.! ix 0 (shiftL 1 n - 1)
    !dist = dijkstraDense (shiftL 1 n) 0 gr
    !inf = 0x3f3f3f3f3f3f3f3f
    !gr = U.create $ do
        adj <- UM.replicate (shiftL 1 n * shiftL 1 n) inf
        rep (shiftL 1 n) $ \i -> do
            UM.write adj (ix i i) 0
        rep (shiftL 1 n) $ \i -> do
            U.forM_ es $ \(e, c) -> do
                UM.modify adj (min c) (ix i (i .|. e))
        return adj
    ix i j = shiftL i n .|. j
    {-# INLINE ix #-}
...
-------------------------------------------------------------------------------
dijkstraDense :: (U.Unbox w, Num w, Ord w, Bounded w) => Int -> Int -> U.Vector w -> U.Vector w
dijkstraDense n src gr | src >= n || U.length gr /= n * n = error "dijkstraDense: Invalid Arguments" | otherwise = U.create $ do { dist <- UM.replicate (n + 1) maxBound; used <- UM.replicate n False; UM.write dist src 0; let { nothing = n}; _v <- UM.replicate 1 0; _dv <- UM.replicate 1 0; fix $ \ loop -> do { UM.unsafeWrite _v 0 nothing; UM.unsafeWrite _dv 0 maxBound; rep n $ \ i -> do { UM.unsafeRead used i >>= \case { False -> do { di <- UM.unsafeRead dist i; dv <- UM.unsafeRead _dv 0; when (di < dv) $ do { UM.unsafeWrite _v 0 i; UM.unsafeWrite _dv 0 di}}; True -> return ()}}; v <- UM.unsafeRead _v 0; when (v /= nothing) $ do { UM.unsafeWrite used v True; dv <- UM.unsafeRead _dv 0; rep n $ \ i -> do { let { di' = dv + U.unsafeIndex gr (v * n + i)}; UM.unsafeModify dist (min di') i}; loop}}; return dist}
