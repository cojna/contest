...
main = do
    n <- readLn @Int
    xs <- U.unfoldrN n (runParser int1) <$> C.getContents
    case solve n xs of
        Just (x, y) -> putStrLn . unwords $ map show [y, x]
        Nothing -> putStrLn "Correct"

solve :: Int -> U.Vector Int -> Maybe (Int, Int)
solve n xs = do
    x <- U.elemIndex (0::Int) freq
    y <- U.elemIndex 2 freq
    return $ (x + 1, y + 1)
  where
    !freq = U.accumulate (+) (U.replicate n 0)
        $ U.map (,1) xs
