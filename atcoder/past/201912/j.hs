...
main = do
    ...
    mat <- U.unfoldrN (h*w) (runParser int) <$> C.getContents
    print $ solve h w mat

solve :: Int -> Int -> U.Vector Int -> Int
solve h w mat
    = U.minimum
    $ U.izipWith3 (\i x y z -> (x + y + z - 2 * mat U.! i))
        dist0 dist1 dist2
  where
    ix x y = x * w + y
    {-# INLINE ix #-}
    ...
    dist0 = dijkstraCSR (ix (h - 1) 0) gr
    dist1 = dijkstraCSR (ix 0 (w - 1)) gr
    dist2 = dijkstraCSR (ix (h - 1) (w - 1)) gr

    !gr = buildSparseGraph (h * w) $ \builder -> do
        rep (h * w) $ \xy -> do
            neighbor4' h w xy $ \nxy -> do
                addDirectedEdge builder (xy, nxy, mat U.! nxy)
