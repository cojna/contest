...
main = do
    ...
    let mat = solve n q qs
    rep n $ \i -> do
        putStrLn . map (bool 'N' 'Y') . U.toList $ U.slice (i * n) n mat
...
solve :: Int -> Int -> U.Vector Q -> U.Vector Bool
solve n q qs = U.create $ do
    mat <- UM.replicate (n * n) False
    U.forM_ qs $ \case
        Q1 a b -> do
            UM.write mat (ix a b) True
        Q2 a -> do
            rep n $ \i -> do
                fia <- UM.read mat (ix i a)
                when fia $ do
                    UM.write mat (ix a i) True
        Q3 a -> do
            buf <- newMutVar []
            rep n $ \i -> when (i /= a) $ do
                rep n $ \x -> do
                    fax <- UM.read mat (ix a x)
                    fxi <- UM.read mat (ix x i)
                    when (fax && fxi) $ do
                        modifyMutVar' buf (i:)
            buf' <- readMutVar buf
            forM_ buf' $ \i -> do
