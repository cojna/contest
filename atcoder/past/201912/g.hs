...
main = do
    n <- readLn @Int
    mat <- fmap (++[replicate n 0]) . forM [1..n-1] $ \i -> do
        let offset = 0 <$ [1..i]
        (offset++) . map read.words <$> getLine
    print $ solve n $ U.fromListN (n * n) $ concat mat

solve :: Int -> U.Vector Int -> Int
solve n mat = maximum $ map (calc.U.fromListN n) cands
  where
    cands = mapM (const [0::Int,1,2]) [1..n]
    calc cand = sum $ do
        k <- [0,1,2]
        [mat U.! (i * n + j)|i<-[0..n-1],j<-[i+1..n-1],cand U.! i == k, cand U.! j == k]
