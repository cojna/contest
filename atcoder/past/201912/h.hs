...
main = do
    ...
    print $ solve n xs q qs

type Q = (Int, Int, Int)
pattern Q1 x a = (1, x, a)
pattern Q2 a = (2, a, 0)
pattern Q3 a = (3, a, 0)
{-# COMPLETE Q1, Q2, Q3 #-}
...
solve :: Int -> U.Vector Int -> Int -> U.Vector Q -> Int
solve n xs0 q qs = runST $ do
    xs <- U.thaw xs0
    fmap ((\(x,_,_,_,_)-> x))
        $ U.foldM'
            (\(!res, !acc2, !acc3, !m2, !m3) -> \case
                Q1 x a
                    | even x -> do
                        fx <- UM.unsafeRead xs x
                        if fx - acc2 - acc3 >= a
                        then do
                            UM.unsafeWrite xs x (fx - a)
                            return (res + a, acc2, acc3, min m2 (fx - a), min m3 (fx - a))
                        else return (res, acc2, acc3, m2, m3)
                    | otherwise -> do
                        fx <- UM.unsafeRead xs x
                        if fx - acc3 >= a
                        then do
                            UM.unsafeWrite xs x (fx - a)
                            return (res + a, acc2, acc3, m2, min m3 (fx - a))
                        else return (res, acc2,acc3,m2, m3)
                Q2 a | m2 >= a -> return (res + a * div (n + 1) 2, acc2 + a, acc3, m2 - a, min m3 $ m2 - a)
                     | otherwise -> return (res, acc2, acc3,m2, m3)
                Q3 a | m3 >= a -> return (res + a * n, acc2, acc3 + a, m2 - a, m3 - a)
                     | otherwise -> return (res, acc2, acc3, m2, m3)
            ) (0, 0, 0,U.minimum $ U.ifilter (\i _ -> even i) xs0,U.minimum xs0) qs
