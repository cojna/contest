...
main = do
    ...
    putStr.unlines.map show $ solve q qs


type Query = (Int, Char, Int)
pattern Q1 :: Char -> Int -> Query
pattern Q1 c x = (1, c, x)
pattern Q2 :: Int -> Query
pattern Q2 d = (2, 'X', d)
{-# COMPLETE Q1, Q2 #-}
...
enqueue :: (a, Int) -> Queue a -> Queue a
enqueue x (Q fs rs) = Q fs (x:rs)


takeQ :: Int -> Queue a -> ([(a, Int)], Queue a)
takeQ = go id
    where
        go acc n (Q ((x,cnt):fs) rs)
            | cnt <= n = go (acc . ((x,cnt):)) (n - cnt) (Q fs rs)
            | otherwise = (acc [(x, n)], Q ((x,cnt-n):fs) rs)
        go acc _ (Q [] []) = (acc [], Q [] [])
        go acc n (Q [] (r:rs)) = go acc n (Q (reverse $ r:rs) [])

solve :: Int -> U.Vector Query -> [Int]
solve _q = go (Q [] []) . U.toList
    where
        go q (Q1 c x:qs) = go (enqueue (c, x) q) qs
        go q (Q2 d:qs) = res : go q' qs
            where
                (cxs, q') = takeQ d q
                !res = sum
                    . map ((\x->x*x).sum.map snd)
                    . L.groupBy ((==) `on` fst)
                    $ L.sort cxs
        go _ [] = []
