...
main = do
    n <- readLn @Int
    xs <- U.unfoldrN (shiftL 1 n) (runParser int) <$> C.getLine
...
succCount :: Player -> Player
succCount p@Player{..} = p{count = count + 1}

predCount :: Player -> Player
predCount p@Player{..} = p{count = count - 1}

solve :: Int -> U.Vector Int -> [Int]
solve n xs = go [] . zipWith (\i x -> Player x 1 i) [0..] $ U.toList xs
    where
        go lose [win] = map snd . L.sort
            $ map (\Player{..}->(playerId, count)) $ predCount win:lose
        go lose win = go' [] [] win
            where
                go' ls ws (x:y:xs)
                    | x > y = go' (y:ls) (x:ws) xs
                    | otherwise = go' (x:ls) (y:ws) xs
                go' ls ws _ = go (ls++lose) $ map succCount ws
