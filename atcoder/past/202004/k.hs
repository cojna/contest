...
main = do
    ...
    print $ solve n cs xs ys

solve :: Int -> U.Vector Char -> U.Vector Int -> U.Vector Int -> Int
solve n cs xs ys
    = U.head
    . U.foldl' step (U.cons 0 $ U.replicate n inf)
    $ U.zip3 cs xs ys
    where
        inf = 3000 * 0x3f3f3f3f
        step dp ('(', a, b) = U.generate (n + 1) $ \i ->
            let x | i > 0 = dp U.! (i - 1)
                  | otherwise = inf
                y = dp U.! i + b
                z | i < n = dp U.! (i + 1) + a
