...
main :: IO ()
main = do
    [n, k, d] <- map read.words <$> getLine
    xs <- U.unfoldrN n (runParser int) <$> C.getLine
    putStrLn . maybe "-1" (unwords.map show) $ solve n k d xs

solve :: Int -> Int -> Int -> U.Vector Int -> Maybe [Int]
solve n k d xs = go [] k 0
    where
        !rmq = buildRMQ $ U.imap (flip (,)) xs
        go stack 0 _ = Just $ reverse stack
        go stack !rest !pos
            | sz > 0 =
                let (m, mi) = queryMin rmq pos (pos + sz)
                in go (m:stack) (rest - 1) (mi + d)
            | otherwise = Nothing
            where
                !len = n - pos
                !sz = len - (rest - 1) * d
