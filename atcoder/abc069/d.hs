...
main = do
    ...
    putStr.unlines.map (unwords.map show.U.toList) $ solve h w xs

solve :: Int -> Int -> U.Vector Int -> [U.Vector Int]
solve h w xs = zipWith ($) (cycle [id, U.reverse]) [U.slice (i * w) w mat| i<-[0..h-1]]
  where
    !mat = U.concatMap (\(i, x) -> U.replicate x (i + 1)) $ U.indexed xs
