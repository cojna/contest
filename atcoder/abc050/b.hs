...
main = do
    n <- readLn @Int
    ts <- U.unfoldrN n (runParser int) <$> C.getLine
    m <- readLn @Int
    pxs <- U.unfoldrN m (runParser $ (,) <$> int1 <*> int) <$> C.getContents
    putStr.unlines.map show.U.toList $ solve n m ts pxs

solve :: Int -> Int -> U.Vector Int -> U.Vector (Int, Int) -> U.Vector Int
solve n m ts pxs = U.map calc pxs
  where
    calc (p, x) = U.sum $ U.imap (\i t -> if i == p then x else t) ts
