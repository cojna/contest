...
main = do
    ...
    rs <- U.unfoldrN n (runParser int1) <$> C.getLine
    es <- U.unfoldrN m (runParser $ (,,) <$> int1 <*> int1 <*> int) <$> C.getContents
    print $ solve n m r rs es

inf :: Int
inf = 0x3f3f3f3f3f3f3f3f

solve :: Int -> Int -> Int -> U.Vector Int -> U.Vector (Int, Int, Int) -> Int
solve n m r (U.toList -> rs) es
    = minimum
    . map (go 0)
    $ L.permutations rs
  where
    go !acc (x:y:ps) = go (acc + U.unsafeIndex mat (x * n + y)) (y:ps)
    go !acc _ = acc
    !mat = U.create $ do
        d <- UM.replicate (n * n) inf
        U.forM_ es $ \(a,b,c) -> do
            UM.write d (a * n + b) c
            UM.write d (b * n + a) c
        warshallFloyd n d
