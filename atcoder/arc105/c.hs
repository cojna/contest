...
main = do
    ...
    ws <- U.unfoldrN n (runParser int) <$> C.getLine
    lvs <- U.unfoldrN m (runParser $ (,) <$> int <*> int) <$> C.getContents
    print . maybe (-1) id $ solve n m ws lvs

solve :: Int -> Int -> U.Vector Int -> U.Vector (Int, Int) -> Maybe Int
solve n m (U.toList -> ws) lvs
    | maximum ws > U.minimum (U.map snd lvs) = Nothing
    | otherwise = Just
        . minimum
        . map calc
        $ L.permutations ws
  where
    (!vs, !ls) = U.unzip . U.modify Intro.sort $ U.map swap lvs
    !cummax = U.scanl' max minBound ls
    query w
        = U.unsafeIndex cummax
        $ binarySearch 0 m $ (w<=) . U.unsafeIndex vs
    ...
    calc ps = go $ map (,0) ps
      where
        ...
        go ((x,dx):xds) = go
            . map snd
            . tail
            $ L.scanl' step (x, (x, dx)) xds
          where
            step (!acc, _) (y, dy) = (acc', (y, dy'))
              where
                !acc' = acc + y
                !l = query acc'
                !dy' = max dy (dx + l)
