...
main = do
    ...
    (lrs, sts) <- U.splitAt d . U.unfoldrN (d + k) (runParser $ (,) <$> int1 <*> int1) <$> C.getContents
    putStr.unlines.map show.U.toList $ solve n d k lrs sts

solve :: Int -> Int -> Int -> U.Vector (Int, Int) -> U.Vector (Int, Int) -> U.Vector Int
solve n d k lrs sts = U.map calc sts
  where
    calc (s, t) = go 0 s $ U.toList lrs
      where
        go !res !pos ((l,r):rest)
            | pos == t = res
            | pos < l || r < pos = go (res + 1) pos rest
            | pos < t = go (res + 1) (min t r) rest
            | otherwise = go (res + 1) (max t l) rest
        go res _ _ = res
