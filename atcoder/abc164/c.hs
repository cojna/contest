...
main = do
    n <- readLn :: IO Int
    bss <- U.fromListN n . map encode.C.lines <$> C.getContents
    print . fst
        . U.foldl' (\(!acc, !prev) x ->
            if prev == x then (acc, x) else (acc + 1, x)
            ) (0, 0)
        $ radixSort64 bss

encode :: B.ByteString -> Word64
encode = B.foldl' (\acc w -> 27 * acc + fromIntegral (w - 96)) 0
