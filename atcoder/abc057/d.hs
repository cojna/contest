...
main = do
    ...
    case solve n a b vs of
        (e, cnt) -> print e >> print cnt


solve :: Int -> Int -> Int -> [Int] -> (Double, Int)
solve n a b (L.sortBy (flip compare) -> vs@(v:_))
    | a == n || notElem k ys = (average xs, 1)
    | all (== k) xs = (fromIntegral k, sum[comb cnt r | r<-[a..min cnt b]])
    | otherwise = (average xs, comb cnt cntX)
  where
    (xs, ys) = splitAt a vs
    k = last xs
    cnt = cntX + cntY
    cntX = length $ filter (==k) xs
    cntY = length $ filter (==k) ys
...
comb :: Int -> Int -> Int
comb (toInteger -> n) (toInteger -> k)
    = fromIntegral $ product[n-k+1..n] `quot` product [1..k]
