...
main = do
    n <- readLn :: IO Int
    (es, qs)<- U.splitAt (2*(n-1)).U.unfoldrN (2*(n-1)+1+200000)(runParser int1) <$> C.getContents
    let q = U.head qs + 1
    putStr.unlines.map show.U.toList $ solve n (tuples2N (n-1) es) (tuples2N q $ U.slice 1 (2*q) qs)

solve :: Int -> U.Vector (Int, Int) -> U.Vector (Int, Int) -> U.Vector Int
solve n es qs = U.map (uncurry query) qs
  where
    !gr = buildUndirectedGraph n es
    !lca = buildLCA gr 0
    query a b = (da - dc) + (db - dc) + 1
      where
        c = queryLCA lca a b
        da = queryDepth lca a
        db = queryDepth lca b
...
buildLCA gr root = runST $ do
    met <- newVecQueue @_ @(Int, Vertex) (2 * numVerticesCSR gr - 1)
...
queryDepth LCA{..} v
    = fst
    $ U.unsafeIndex
        (V.unsafeIndex (getSparseTable rmqLCA) 0)
        (U.unsafeIndex firstIndexLCA v)
{-# INLINE queryDepth #-}
...
buildSparseTable :: forall (f :: * -> *) a . (U.Unbox a, Monoid (f a), Coercible (f a) a) => U.Vector a -> SparseTable f a
buildSparseTable vec = SparseTable . V.scanl' (\ acc i -> U.zipWith (coerce op) acc $ U.drop i acc) vec $ V.iterateN (floorLog2 $ U.length vec) (* 2) 1 where { op :: f a -> f a -> f a; op = mappend}
queryST :: forall (f :: * -> *) a . (U.Unbox a, Monoid (f a), Coercible (f a) a) => SparseTable f a -> Int -> Int -> a
queryST st l r | l < r = (coerce op) x y | otherwise = error $ "queryST l: " ++ shows l "r: " ++ show r where { op :: f a -> f a -> f a; op = mappend; logStep = floorLog2 $ r - l; row = V.unsafeIndex (getSparseTable st) logStep; x = U.unsafeIndex row l; y = U.unsafeIndex row $ r - unsafeShiftL 1 logStep}
