...
main = do
    ...
    putStr.unlines.map show.U.toList $ solve n a b xs

solve :: Int -> Int -> Int -> U.Vector Int -> U.Vector IntMod
solve _ 1 _ xs = U.map IntMod $ U.modify Intro.sort xs
solve 1 a b xs = U.map (\x-> (IntMod a ^ b) * IntMod x) xs
solve n a b xs
    | b <= b0 = solveNaive n a b xs
    | otherwise = U.map (\x -> (IntMod a ^ q) * IntMod x) xsr
        U.++  U.map (\x -> (IntMod a ^ (q + 1)) * IntMod x) xsl
  where
    !mm = U.maximum xs
    !b0 = U.sum $ U.map (\x -> pred . length . takeWhile (<= mm) $ iterate (*a) x) xs
    !xs' = U.modify Intro.sort $ U.map (\x -> last . takeWhile (<= mm) $ iterate (*a) x) xs
    (q, r) = divMod (b - b0) n
    (xsl, xsr) = U.splitAt r xs'

solveNaive :: Int -> Int -> Int -> U.Vector Int -> U.Vector IntMod
solveNaive n a b = go b . L.sort . U.toList
  where
    go 0 xs = U.fromListN n $ map intMod $ L.sort xs
    go !rest (x:xs) = go (rest - 1) $! L.insert ax xs
      where
        !ax = a * x
