...
jankens :: Int -> String -> String
jankens n cs = take n . go $ cycle cs
  where
    go (x:y:cs) = let !z = janken x y in z : go cs

solve :: Int -> Int -> String -> Char
solve _ 0 (c:_) = c
solve !n !k cs = solve n (k - 1) $ jankens n cs
