...
main :: IO ()
main = do
    cs <- U.unfoldr C.uncons <$> C.getLine
    print $ solve cs
...
solve cs = runST $ do
    dp <- UM.replicate (13 * (U.length cs + 1)) 0
    UM.write dp (ix 0 0) 1
    flip U.imapM cs $ \i c -> do
        case c of
            '?' -> do
                rep 10 $ \k -> do
                    rep 13 $ \j -> do
                        !dpij <- UM.unsafeRead dp (ix i j)
                        UM.unsafeModify dp (+% dpij) (ix (i+1) (rem (10 * j + k) 13))
            d -> do
                let k = digitToInt d
                rep 13 $ \j -> do
                    !dpij <- UM.unsafeRead dp (ix i j)
                    UM.unsafeModify dp (+% dpij) (ix (i+1) (rem (10 * j + k) 13))
    UM.read dp (ix (U.length cs) 5)
