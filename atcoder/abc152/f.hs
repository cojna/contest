...
main = do
    ...
    es <- replicateM (n-1) $ do
        [!a, !b] <- map (subtract 1.read).words <$> getLine
        return $! (a, b)
    m <- readLn
    uvs <- replicateM m $ do
        [!u, !v] <- map (subtract 1.read).words <$> getLine
        return $! (u, v)
    print $ solve n es m uvs

solve :: Int -> [(Int, Int)] -> Int -> [(Int, Int)] -> Int
solve n es m uvs = U.sum . U.generate (2 ^ m) $ \s ->
    let !s' = F.foldl'(.|.)0[paths U.! i|i<-[0..m-1],testBit s i]
        !f = 2 ^ (n - 1 - popCount s')
    in if even (popCount s) then f else -f
  where
    paths :: U.Vector Int
    !paths = U.fromList [mkPath u v | (u,v)<-uvs]

    !gr = buildUndirectedGraph n (U.fromList es)

    dict :: M.Map (Vertex, Vertex) Int
    !dict = M.fromList . map (second bit) $ zip es [0..] ++ zip (map swap es) [0..]

    mkPath :: Vertex -> Vertex -> Int
    mkPath start goal = fix (\dfs !p !acc !v ->
        if v == goal
        then acc
        else U.foldl' max 0
            . U.map (\nv -> dfs v (acc .|. (dict M.! (v, nv))) nv)
            . U.filter (/= p)
            $ gr `adj` v
        ) (-1) 0 start
