...
solve n 1 = fromIntegral $ n * (n + 1) `div` 2
solve n k
    = sum
    . map (\(g, x) -> x * fromIntegral (quot k g))
    . IM.toList
    $ F.foldl'
        (\acc p->
            F.foldl'
                (\im d -> case IM.lookup (d * p) im of
                    Just v -> IM.adjust (subtract v) d im
                    Nothing -> im
                )
                acc
                (divisors k)
        )
        (IM.fromList [(d, calc d)|d<-divisors k])
        (map head.L.group $ primeFactors k)
  where
    calc :: Int -> IntMod
    calc d = fromIntegral $ d * (q * (q + 1) `div` 2)
      where
        (q, r) = quotRem n d
