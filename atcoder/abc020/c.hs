                ...
                when (d == dxy) $ do
                    neighbor4 x y $ \nx ny -> do
                        when (inGrid h w nx ny) $ do
                            let dnxny' | mat U.! ix nx ny == '#' = d + k
                                       | otherwise = d + 1
                            dnxny <- UM.read dist (ix nx ny)
                            when (dnxny' < dnxny) $ do
                                UM.write dist (ix nx ny) dnxny'
                                insertBH (dnxny', ix nx ny) heap
                loop
            Nothing -> return ()
    (<= t) <$> UM.read dist goal
  where
