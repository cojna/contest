...
solve n (U.toList -> cs) gx gy = and
    [ IS.member gx $ F.foldl' step (IS.singleton $ length fs) xs
    , IS.member gy $ F.foldl' step (IS.singleton 0) ys
    ]
  where
    (xs, ys) = simulate fts
    (fs, fts) = span (== 'F') cs
...
simulate = bimap reverse reverse . goX [] []
  where
    goX resX resY ('F':cs) = goX (length fs:resX) resY fts
      where
        (fs,fts) = span (=='F') $ 'F':cs
    goX resX resY ('T':cs) = goY resX resY cs
    goX resX resY _ = (resX, resY)

    goY resX resY ('F':cs) = goY resX (length fs:resY) fts
      where
        (fs,fts) = span (=='F') $ 'F':cs
