...
main = runSolver $ do
    n <- int
    bss <- vectorH @V.Vector n bytestring
    return . maybe "satisfiable" B.byteString $ solve n bss

solve :: Int -> V.Vector C.ByteString -> Maybe C.ByteString
solve n bss = (bss V.!) <$> V.findIndex p bss
    where
        !set = V.foldl' (flip IS.insert) IS.empty $ V.map hash bss
        p bs = h > 0 && IS.member h set && IS.member  (-h) set
            where
                !h = hash bs
...
hash bs
    | C.head bs == '!' = negate . f $ C.tail bs
    | otherwise = f bs
    where
        f =  C.foldl' (\acc c -> acc * 26 + conv c) 0

conv :: Char -> Int
conv c = ord c - ord 'a' + 1
