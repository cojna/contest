...
main = runSolver $ do
    n <- int
    xys <- vectorH @U.Vector n (flip (,) <$> int <*> int)
    return . showLnB $ solve n xys

solve :: Int -> U.Vector (Int, Int) -> Int
solve n xys = binarySearch 0 n $ \i ->
    let (xs, ys) = U.splitAt i ds
        !sumX = U.sum $ U.map (uncurry (+)) xs
        !sumY = U.sum $ U.map snd ys
    in sumX > sumY
    where
        !ds = U.modify (Intro.sortBy (flip (comparing (\(x,y)->x+2*y)))) xys
