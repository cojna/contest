...
main = runSolver $ do
    n <- int
    es <- vectorH @U.Vector (n-1) ((,) <$> int1 <*> int1)
    q <- int
    qs <- vectorH @U.Vector q ((,,) <$> int <*> int1 <*> int)
    return . unlinesB B.intDec $ solve n es q qs

solve :: Int -> U.Vector (Int, Int) -> Int -> U.Vector (Int, Int, Int) -> U.Vector Int
solve n es q qs = U.create $ do
    dp <- UM.replicate n 0
    U.forM_ qs $ \case
        (1, ei, x) -> do
            let (a, b) = es U.! ei
            ...
            then do
                UM.modify dp (+x) root
                UM.modify dp (subtract x) b
            else UM.modify dp (+x) a
        (2, ei, x) -> do
            let (b, a) = es U.! ei
                !da = queryDepth lca a
                !db = queryDepth lca b
