...
main = do
    ...
    fs <- U.unfoldrN n (runParser int1) <$> C.getContents
    print $ solve n m fs

solve :: Int -> Int -> U.Vector Int -> IntMod
solve n m fs = runST $ do    
    dp <- newFenwickTree (n + 1)
    addAt 0 1 dp
    prev <- UM.replicate m 0
    lastDup <- UM.replicate 1 0
    U.forM_ (U.imap ((,).(+1)) fs) $ \(i, f) -> do    
        p <- UM.read prev f
        ld <- UM.read lastDup 0
        when (p > 0) $ do
            UM.modify lastDup (max p) 0
        UM.write prev f i
        cum <- sumFromTo (max p ld) i dp        
        addAt i cum dp
    sumFromTo n (n + 1) dp
