...
main = do
    ...
    xvs <- U.unfoldrN n (runParser $ (,) <$> int <*> int) <$> C.getContents
    print $ solve n c xvs
...
solve' :: Int -> Int -> U.Vector (Int, Int) -> Int
solve' n c xvs = U.maximum $ U.zipWith (+) cumCW cumCCW
  where
    cumCW = U.map (\(x, acc) -> acc - x)
        $ U.scanl' (\(_, !acc) (x, v) -> (x, acc + v)) (0, 0) xvs
    cumCCW = U.scanr' max 0
        . U.init
        . U.map (\(x, acc) -> acc - 2 * (c - x))
        $ U.scanr' (\(x, v) (_, !acc) -> (x, acc + v)) (0, 0) xvs
