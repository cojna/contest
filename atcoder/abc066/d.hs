...
main = do
    n <- readLn @Int
    xs <- U.unfoldrN (n + 1) (runParser int1) <$> C.getLine
    putStr.unlines.map show.U.toList $ solve n xs

solve :: Int -> U.Vector Int -> U.Vector IntMod
solve n xs = U.generate (n + 1) $ \i -> calc (i + 1)
  where
    Just !dup = U.elemIndex (2 :: Int)
        . U.accumulate (+) (U.replicate (n + 1) 0)
        $ U.map (,1) xs
    !l = U.head $ U.elemIndices dup xs
    !r = U.last $ U.elemIndices dup xs

    calc k
        | k - 1 <= len = comb (n + 1) k - comb len (k - 1)
        | otherwise = comb (n + 1) k
      where
        len = l + (n + 1) - r - 1
