...
main = do
    ...
    print $ mod (solve n k xs) MOD

solve :: Int -> Int -> U.Vector Int -> IntMod
solve n k xs
    | not canPositive && canZero = 0
    | not canPositive = (IntMod 0-).U.product. U.map coerce . U.take k . radixSortInt $ poss U.++ negs
    | otherwise = go 0 1 (U.toList poss) (U.toList negs)
  where
    !poss = U.reverse . radixSortInt $ U.filter (>0) xs
    !negs = U.reverse . radixSortInt . U.map negate $ U.filter (<0) xs
    !zeros = U.filter (== 0) xs
    !pos = U.length poss
    !neg = U.length negs
    !zero = U.length zeros
    ...
    canZero = zero > 0
    canPositive = pos + 2 * div (min k neg) 2 >= k

    go !cnt !res (x0:x1:xs) (y0:y1:ys)
        | cnt == k = res
        | cnt + 1 == k = res * IntMod x0
        | x0 * x1 < y0 * y1 = go (cnt + 2) (res * IntMod (y0 * y1 `rem` MOD)) (x0:x1:xs) ys
        | otherwise = go (cnt + 1) (res * IntMod x0) (x1:xs) (y0:y1:ys)
    go cnt res xs [] = F.foldl' (*) res . coerce @[Int] @[IntMod] $ take (k - cnt) xs
    go cnt res xs [_] = F.foldl' (*) res . coerce @[Int] @[IntMod] $ take (k - cnt) xs
    go cnt res [] ys = F.foldl' (*) res . coerce @[Int] @[IntMod] $ take (k - cnt) ys
    go cnt res [x] ys
        | cnt == k = res
        | even (k - cnt) = F.foldl' (*) res . coerce @[Int] @[IntMod] $ take (k - cnt) ys
        | otherwise = F.foldl' (*) (res * IntMod x) . coerce @[Int] @[IntMod] $ take (k - (cnt + 1)) ys
