...
main = do
    ...
    cs <- U.unfoldrN (n+1) C.uncons <$> C.getLine
    case solve n m cs of
        Nothing -> print (-1)
...
solve n m cs = go [] n
  where
    go res !pos
        | pos == 0 = Just res
        | otherwise = case [i | i<-[max 0 (pos-m)..pos-1], U.unsafeIndex cs i == '0'] of
            [] -> Nothing
            (pos':_) -> let !d = pos - pos' in go (d:res) pos'
