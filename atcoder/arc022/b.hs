...
data Q = Q !Int !IS.IntSet [Int] [Int]

push :: Int -> Q -> Q
push x (Q cnt set fs rs) = Q (cnt + 1) (IS.insert x set) fs (x:rs)

pop :: Q -> Q
pop (Q cnt set (f:fs) rs) = Q (cnt - 1) (IS.delete f set) fs rs
pop (Q 0 _ _ _)  = Q 0 IS.empty [] []
pop (Q cnt set [] rs) = pop $ Q cnt set (reverse rs) []

solve :: Int -> U.Vector Int -> Int
solve n xs0 = maximum $ go (Q 0 IS.empty [] []) $ U.toList xs0
  where
    go q@(Q cnt set _ _) (x:xs)
        | IS.notMember x set = (cnt + 1) : go (push x q) xs
        | otherwise = go (pop q) (x:xs)
    go _ [] = [0]
