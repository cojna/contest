...
main = do
    ...
    let (xs, ys) = solve n ps
    mapM_ (putStrLn.unwords.map show.U.toList) [xs, ys]

solve :: Int -> U.Vector Int -> (U.Vector Int, U.Vector Int)
solve n ps = (xs, ys)
  where
    m = 23456
    ips = U.accumulate (flip const) (U.replicate n 0) $ U.imap (flip (,)) ps
    xs = U.map succ . U.zipWith (+) ips $ U.generate n (*m)
    ys = U.generate n ((*m) . (n-))
