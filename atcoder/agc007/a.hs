...
main = do
    ...
    mat <- U.unfoldrN (h*w) (runParser char)
        ...
        <$> C.getContents
    putStrLn.bool"Impossible""Possible" $ solve h w mat

solve :: Int -> Int -> U.Vector Char -> Bool
solve h w mat = cnt == h + w - 1 && and [isValid x y|x<-[0..h-1],y<-[0..w-1]]
  where
    cnt = U.length $ U.elemIndices '#' mat
    ix x y = x * w + y
    isValid 0 0 = True
    isValid x 0
        = mat U.! ix x 0 == '.'
